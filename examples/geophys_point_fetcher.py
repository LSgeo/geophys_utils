...
class GeophysPointFetcher(object):
    ...
    def dataset_metadata_generator(
        ...
        for metadata_dict in self._metadata:
            ...
            try:
                if not (
                    float(metadata_dict["geospatial_lon_min"]) <= bounding_box[0]
                ...
                logger.warning(
                    "Unable to determine bounding box for {}".format(
    ...
    def point_data_generator(
        ...
        for metadata_dict in self.dataset_metadata_generator(
            ...
            try:
                ...
                point_indices = np.where(np.logical_and(spatial_mask, line_mask))[0]
                ...
                for variable_name in netcdf_line_utils.point_variables:
                    if not variable_names or variable_name in variable_names:
                        ...
                        dataset_dict["values"][variable_name] = nc_dataset.variables[
                            variable_name
                        ][:][point_indices]
...
def main():
    ...
    def parse_bounding_box_string(bounding_box_string):
        ...
        try:
            bounding_box = [
                float(value)
                for value in re.sub("\s", "", bounding_box_string).split(",")
            ]
            ...
            raise Exception(
                'Invalid bounding box string "{}"'.format(bounding_box_string)
            )
    ...
    for dataset_dict in gpf.point_data_generator(
