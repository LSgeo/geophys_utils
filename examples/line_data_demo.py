...
def get_netcdf_datasets(
    ...
    netcdf_list = [
        distribution["url"] for distribution in cswu.get_netcdf_urls(record_list)
    ]
...
def dataset_point_generator(
    ...
    for dataset in dataset_list:
        ...
        try:
            ...
            reprojected_bounds = get_reprojected_bounds(
            ...
            if flight_lines_only:
                ...
                line_numbers = nc_dataset.variables["line"][
                    nc_dataset.variables["lineType"][:] == 2
                ]
                line_mask = np.zeros(
                    shape=nc_dataset.variables[variable_name].shape, dtype=bool
                )
            ...
            selection_mask = np.logical_and(
                netcdf_line_utils.get_spatial_mask(reprojected_bounds), line_mask
            )
            selection_count = np.count_nonzero(selection_mask)
            print(
                f"{selection_count}/{len(mag_awags_variable)} points found in bounding box"
            )
            ...
            coordinates = np.array(
                transform_coords(
                    netcdf_line_utils.xycoords[selection_mask],
...
def get_points_from_dict(dataset_point_dict):
    ...
    assert (
        all_values.shape[0] == all_coordinates.shape[0]
    ), "Mismatched coordinate and value counts"
    print(
        f"A total of {all_values.shape[0]} points were read from {len(dataset_point_dict)} line datasets"
    )

    return all_coordinates, all_values


def get_points_from_datasets(
    dataset_list,
    wgs84_bounds,
    variable_name,
    coordinate_wkt,
    ...
    all_coordinates = []
    all_values = []
    for dataset, coordinates, values in read_datasets(
        dataset_list,
        wgs84_bounds,
        variable_name,
        coordinate_wkt,
    ...
    print(
        f"A total of {all_values.shape[0]} points were read from {len(dataset_list)} line datasets"
    )
...
def grid_points(
    ...
    pixel_centre_bounds = (
        ...
        round(
            math.floor(grid_bounds[2] / grid_resolution - 1.0) * grid_resolution
            + grid_resolution,
            6,
        ),
        round(
            math.floor(grid_bounds[3] / grid_resolution - 1.0) * grid_resolution
            + grid_resolution,
            6,
        ),
    )
    ...
    print("Computing spatial mask")
    spatial_subset_mask = np.logical_and(
        np.logical_and(
            (grid_bounds[0] <= grid_coordinates[:, 0]),
            (grid_coordinates[:, 0] <= grid_bounds[2]),
        ),
        np.logical_and(
            (grid_bounds[1] <= grid_coordinates[:, 1]),
