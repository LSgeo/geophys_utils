...
def plot_point_dataset(
    ...
    def rescale_array(input_np_array, new_range_min=0, new_range_max=1):
        ...
        scaled_np_array = (
            (input_np_array - old_min) / old_range * new_range
        ) + new_range_min
    ...
    if utm_bbox is None:
        ...
        spatial_mask = np.logical_and(
            np.logical_and(
                (utm_bbox[0] <= utm_coords[:, 0]), (utm_coords[:, 0] <= utm_bbox[2])
            ),
            np.logical_and(
                (utm_bbox[1] <= utm_coords[:, 1]), (utm_coords[:, 1] <= utm_bbox[3])
            ),
        )
        utm_coords = utm_coords[spatial_mask]

    print(
        "{} points in UTM bounding box: {}".format(
    ...
    x_ticks = np.arange(
        (utm_bbox[0] // x_increment + 1) * x_increment, utm_bbox[2], x_increment
    )
    y_ticks = np.arange(
        (utm_bbox[1] // y_increment + 1) * y_increment, utm_bbox[3], y_increment
    )
    ...
    try:  # not all variables have units. These will fail on the try and produce the map without tick labels.
        ...
        cb.ax.set_yticklabels(
            [str(np.min(variable[spatial_mask])), str(np.max(variable[spatial_mask]))]
    ...
    print(
        "1D Point variables:\n\t{}".format(
            "\n\t".join(
                [
                    key
                    for key, value in netcdf_dataset.variables.items()
