...
class TileGridder(object):
    ...
    GDA94_CRS_WKT = get_wkt_from_spatial_ref(
        get_spatial_ref_from_wkt("EPSG:4283")
    )  # Defaults to GDA94
    ...
    def __init__(
        ...
        self.filter_variable_name = (
            filter_variable_name or TileGridder.DEFAULT_FILTER_VARIABLE_NAME
        )
        self.filter_value_list = (
            filter_value_list or TileGridder.DEFAULT_FILTER_VALUE_LIST
        )
        ...
        if (
            tile_extra is None
        ):  # Expand bounds by TileGridder.DEFAULT_TILE_EXPANSION_PERCENT percent each side
            self.expanded_grid_bounds = [
                grid_bounds[0]
                - (grid_bounds[2] - grid_bounds[0])
                * TileGridder.DEFAULT_TILE_EXPANSION_PERCENT,
                grid_bounds[1]
                - (grid_bounds[3] - grid_bounds[1])
                * TileGridder.DEFAULT_TILE_EXPANSION_PERCENT,
                grid_bounds[2]
                + (grid_bounds[2] - grid_bounds[0])
                * TileGridder.DEFAULT_TILE_EXPANSION_PERCENT,
        ...
        self.expanded_gda94_grid_bounds = self.reproject_bounds(
            self.expanded_grid_bounds, self.grid_crs_wkt, TileGridder.GDA94_CRS_WKT
        )
    ...
    def dataset_values(self):
        ...
        if self._dataset_values is None:
            self._dataset_values = {
                ...
                for dataset, dataset_value_dict in self.dataset_value_generator(
    ...
    def dataset_value_generator(
        ...
        for dataset in dataset_list:
            try:
                ...
                print(
                    "{}/{} points found in expanded bounding box for {}".format(
                ...
                for variable_name in variable_name_list:
                    ...
                    if (
                        ...
                        dataset_value_dict[variable_name] = (
    ...
    def grid_points(
        ...
        pixel_centre_bounds = (
            round(
                (math.floor(grid_bounds[0] / grid_resolution) + 0.5) * grid_resolution,
                6,
            ),
            round(
                (math.floor(grid_bounds[1] / grid_resolution) + 0.5) * grid_resolution,
                6,
            ),
            round(
                (math.floor(grid_bounds[2] / grid_resolution) - 0.5) * grid_resolution,
                6,
            ),
            round(
                (math.floor(grid_bounds[3] / grid_resolution) - 0.5) * grid_resolution,
                6,
            ),
        )
                        ...
                        ",".join(
                            [
                                ...
                                ",".join(
                                    [
                                        str(ordinate)
                                        for ordinate in arrays["coordinates"][
                                            point_index
                                        ]
                                    ]
                                ),
...
def main():
    ...
    def quote_delimitedtext(text, delimiter, quote_char='"'):
        ...
        if delimiter in text or quote_char in text or re.search("\s", text):
            if delimiter == ",":  # Use double quote to escape quote character for CSV
                return (
                    quote_char
                    + text.replace(quote_char, quote_char + quote_char)
                    + quote_char
                )
            else:  # Use backslash to escape quote character for tab or space delimited text
                return (
                    quote_char
                    + text.replace(quote_char, "\\" + quote_char)
                    + quote_char
                )
        else:
            return text
    ...
    parser.add_argument(
        "-d", "--data_variable", help="data variable name", type=str, required=True
    )
    parser.add_argument(
        "-r", "--grid_resolution", help="grid resolution", type=float, required=True
    )
    ...
    parser.add_argument(
        "-x",
        "--tile_extra",
        help="absolute extra per side for tiling. Defaults to 5% extra on each side",
        type=float,
        required=False,
    )
    ...
    grid_crs_wkt = get_wkt_from_spatial_ref(
        get_spatial_ref_from_wkt(args.crs)
    )  # Defaults to GDA94
    ...
    tile_extra = (
        args.tile_extra
    )  # Absolute extra per side. Defaults to 5% extra on each side
    ...
    for ll_point in itertools.product(
        *[
            np.arange(
                grid_bounds[0 + dim_index],
                grid_bounds[2 + dim_index],
                tile_size[dim_index],
            )
        ...
        tile_path = os.path.join(
            output_dir,
            "tiles",
            "_".join([grid_variable_name] + [str(ordinate) for ordinate in tile_bounds])
            + ".tif",
        )
        ...
        if not os.path.isfile(
            ...
            try:
                ...
                print(
                    "Unable to create dataset list file {}: {}".format(
                        dataset_list_path, e
                    )
                )
        ...
        if args.process and os.path.isfile(dataset_list_path):
            ...
            if os.path.isfile(
                ...
                try:
                    ...
                    print(
                        "Finished read dataset list file {}".format(dataset_list_path)
                    )
                except Exception as e:
                    print(
                        "Unable to read dataset list file {}: {}".format(
                            dataset_list_path, e
                        )
                    )
                    continue
            ...
            if not os.path.isfile(
                ...
                try:
                    ...
                    print(
                        "Unable to create point file {}: {}".format(point_list_path, e)
                    )
                    continue
            ...
            with open(point_list_path, "r") as csv_file:
                ...
                for line in csv_file.readlines():
                    ...
                    point_coordinates.append(
                        tuple(float(ordinate) for ordinate in line_values[1:3])
                    )
