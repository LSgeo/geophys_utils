...
class NearestGeophysPointFinder(object):
    ...
    def __init__(self, metadata_csv_path=None):
        ...
        with open(metadata_csv_path) as metadata_csv_file:
            self._metadata_keys = [
                key.lower() for key in csv.DictReader(metadata_csv_file).fieldnames
            ]
    ...
    def dataset_metadata_generator(
        ...
        for metadata_dict in self._metadata:
            ...
            for coordinate in coordinate_list:
                point_in_dataset = (
                    float(metadata_dict["geospatial_lon_min"])
                    <= (coordinate[0] + max_distance)
                    and float(metadata_dict["geospatial_lat_min"])
                    <= (coordinate[1] + max_distance)
                    and float(metadata_dict["geospatial_lon_max"])
                    >= (coordinate[0] - max_distance)
                    and float(metadata_dict["geospatial_lat_max"])
                    >= (coordinate[1] - max_distance)
                )
    ...
    def get_nearest_point_data(
        ...
        for metadata_dict in self.dataset_metadata_generator(
            ...
            try:
                ...
                if variable_names and not (
                    set(variable_names) & set(netcdf_line_utils.point_variables)
                ):
                    logger.debug(
                ...
                if flight_lines_only:
                    ...
                    for (
                        _line_number,
                        single_line_mask,
                    ) in netcdf_line_utils.get_line_masks(line_numbers):
                ...
                for coordinate in coordinate_list:
                    ...
                    if len(point_indices):
                        ...
                        for found_index in range(len(point_indices)):
                            ...
                            for variable_name in netcdf_line_utils.point_variables:
                                if (
                                    ...
                                    point_metadata_dict[variable_name] = (
                                        nc_dataset.variables[variable_name][
                        ...
                        logger.info(
                            "No points near {} found in {}".format(coordinate, nc_path)
                        )
...
def JW_metadata_filter(metadata_dict):
    ...
    try:
        ...
        if datetime.strptime(
            metadata_dict["acquisition_start_date"], "%d/%m/%Y"
        ) < datetime.strptime("01/01/1981", "%d/%m/%Y"):
            return False

        # Only accept GA/BMR/AGSO datasets between 1981 and 1992
        if datetime.strptime(
            metadata_dict["acquisition_start_date"], "%d/%m/%Y"
        ) < datetime.strptime("01/01/1992", "%d/%m/%Y") and metadata_dict[
            "client"
        ] not in [
        ...
        logger.warning(
            "WARNING: Unhandled date format: {}".format(
                metadata_dict["acquisition_start_date"]
            )
        )
        return False
...
def main():
    ...
    def parse_coordinate_list_string(coordinate_list_string):
        ...
        try:
            ...
            while processing_string:
                match = re.match(
                    "\((([\+\-.0-9]+),(\s*)([\+\-.0-9]+))\)(,(.*))*", processing_string
                )
                if match:
                    coordinate_list.append(
                        (float(match.group(2)), float(match.group(4)))
                    )
            ...
            raise Exception(
                'Invalid coordinate list string "{}"'.format(coordinate_list_string)
            )
    ...
    variable_names = (
        [variable_name.strip() for variable_name in args.variable_names.split(",")]
    ...
    point_result_dict = ngpf.get_nearest_point_data(
    ...
    for coordinate in sorted(point_result_dict.keys()):
        ...
        logger.info(
            "A total of {} points were found near {} in {} datasets".format(
                len(point_list), coordinate, len(dataset_set)
            )
        )
