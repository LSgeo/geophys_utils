...
POINT_STRIDE = (
    10  # Number of points to skip when computing median sample spacing (1 = no skip)
)
...
class NetCDFLineUtils(NetCDFPointUtils):
    ...
    def get_line_masks(
        ...
        if subset_mask is not None:
            line_number_subset = line_number_subset[
                np.isin(
                    line_number_subset,
                    self.line[np.unique(self.line_index[subset_mask])],
                )
            ]  # Exclude lines not in subset
        else:
            line_number_subset = line_number_subset[
                np.isin(line_number_subset, self.line)
            ]  # Exclude bad line numbers

        line_mask = np.zeros(
            shape=self.line_index.shape, dtype=bool
        )  # Keep re-using same in-memory array
    ...
    def get_lines(
        ...
        for line_number, line_mask in self.get_line_masks(
            line_numbers=line_numbers,
            ...
            if line_point_count:  # This test should be redundant
                ...
                if subsampling_distance:
                    line_length = pdist(
                        [
                            self.xycoords[point_indices[0]],
                            self.xycoords[point_indices[-1]],
                        ]
                    )[0]
                    ...
                    stride = max(
                        ...
                        int(
                            line_point_count
                            / max(1, line_length / subsampling_distance)
                        ),
                    )
                    ...
                    subset_indices = np.unique(
                        np.concatenate(
                            (
                                np.arange(0, line_point_count, stride),
                    ...
                    logger.debug(
                        "Subset of line {} has {} points".format(
                            line_number, len(subset_indices)
                        )
                    )
                ...
                for variable_name in variables:
                    line_dict[variable_name] = self.netcdf_dataset.variables[
                        variable_name
                    ][point_indices]
    ...
    def get_line_values(self):
        ...
        if line_variable.shape:  # Multiple lines
            ...
            line_values = line_variable[
                :
            ]  # Should be small enough to retrieve in one hit

        else:  # Scalar - only one line
            line_values = line_variable[:].reshape(
                (1,)
            )  # Change scalar to single-element 1D array
    ...
    def get_line_index_values(self):
        ...
        if len(self.netcdf_dataset.variables["line"]):  # Multiple lines
            ...
            line_indices = np.zeros(shape=(self.point_count,), dtype=np.int8)
        ...
        if self.enable_disk_cache:
            if os.path.isfile(self.cache_path):
                ...
                if "line" in cache_dataset.variables.keys():
                    ...
                    logger.debug(
                        "Read {} lines from cache file {}".format(
                            line.shape[0], self.cache_path
                        )
                    )
                else:
                    logger.debug(
                        "Unable to read line variable from netCDF cache file {}".format(
                            self.cache_path
                        )
                    )
                ...
                if "line_index" in cache_dataset.variables.keys():
                    ...
                    logger.debug(
                        "Read {} line_indices from cache file {}".format(
                            line_index.shape[0], self.cache_path
                        )
                    )
                else:
                    logger.debug(
            ...
            if line is None or line_index is None:
                ...
                if "point" not in cache_dataset.dimensions.keys():
                    cache_dataset.createDimension(
                        dimname="point", size=line_index.shape[0]
                    )
                ...
                cache_dataset.variables["line_index"][
                    :
                ] = line_index  # Write lines to cache file

                cache_dataset.close()
                logger.debug(
                    "Saved {} lines for {} points to cache file {}".format(
                        line.shape[0], line_index.shape[0], self.cache_path
                    )
                )
    ...
    def line(self):
        ...
        if self.enable_memory_cache and self._line is not None:
            ...
            if line is not None:
                ...
                line = self.get_line_values()
                logger.debug(
                    "memcached key not found. Adding entry with key {}".format(
                        line_cache_key
                    )
                )
                self.memcached_connection.add(line_cache_key, line)
    ...
    def get_line_sample_points(self, line_divisions=10):
        ...
        for line_index in range(self.netcdf_dataset.dimensions["line"].size):
            line_indices = np.where(
                self.netcdf_dataset.variables["line_index"][:] == line_index
            )[0]
            logger.debug(
                "Sampling line index {} with {} points".format(
                    line_index, len(line_indices)
                )
            )
            ...
            if not np.count_nonzero(valid_coord_mask):  # No valid coordinates in line
                logger.debug(
                    "No valid coordinates found in line index {}".format(line_index)
                )
                continue
    ...
    def get_multi_line_string(self, to_wkt=None, tolerance=0):
        ...
        for line_index in range(len(line_start_indices)):
            line_slice = slice(
                line_start_indices[line_index], line_end_indices[line_index] + 1
            )
            line_vertices = self.xycoords[line_slice]
            line_vertices = line_vertices[
                ~np.any(np.isnan(line_vertices), axis=1)
            ]  # Discard null coordinates
            if (
                len(line_vertices) >= 2
    ...
    def get_concave_hull(
        ...
        ), "buffer_distance must be greater than zero if number of polygons is limited"  # Avoid endless recursion
        ...
        def get_offset_geometry(
            ...
            offset_geometry = offset_geometry.buffer(
            ...
            if type(offset_geometry) == MultiPolygon:
                ...
                raise ValueError(
                    "Unexpected type of geometry: {}".format(type(offset_geometry))
                )
            ...
            if (
                (
                    ...
                    and len(offset_geometry) > max_polygons
                )
                or (
                    max_vertices
                    and type(offset_geometry) == MultiPolygon
        ...
        multi_line_string = self.get_multi_line_string(
            to_wkt=to_wkt, tolerance=tolerance
        )
    ...
    def set_global_attributes(
        ...
        try:
            super().set_global_attributes(
                compute_shape=False
            )  # Call NetCDFPointUtils.set_global_attributes

            attribute_dict = {}
            if compute_shape:
                print("compute shape in lines_utils")
                wkt_polygon = shapely.wkt.dumps(
                    Polygon(self.get_concave_hull()),
                ...
                if clockwise_polygon_orient:
                    logger.debug(
                        "setting 'geospatial_bounds' as a clockwise orientation"
                    )
                    wkt_polygon = shapely.geometry.polygon.orient(Polygon(pol), -1.0)
                else:  # reverse polygon coordinates - anti-clockwise
                    logger.debug(
                        "Keeping default setting 'geospatial_bounds' to anticlockwise orientation"
                    )
                    wkt_polygon = shapely.geometry.polygon.orient(Polygon(pol), 1.0)
            ...
            if compute_median_sample_spacing:
                try:
                    ...
                    _utm_wkt, utm_coordinate_array = utm_coords(
                        self.xycoords[::POINT_STRIDE], self.wkt
                    )
                    # logger.debug('utm_coordinate_array = {}'.format(utm_coordinate_array))
                    sample_distance_array = coords2distance(
                        utm_coordinate_array
                    )  # Absolute distances of sample points along line
                    sample_distance_array = (
                        sample_distance_array[1:] - sample_distance_array[:-1]
                    )  # Compute relative distances from absolute distances
                    # logger.debug('sample_distance_array = {}'.format(sample_distance_array))
                    median_sample_spacing_m = round(
                        np.nanmedian(sample_distance_array) / POINT_STRIDE, 1
                    )
                    assert not np.isnan(
                        median_sample_spacing_m
                    ), "median_sample_spacing_m is Not a Number (NaN)"
                    assert not np.isinf(
                        median_sample_spacing_m
                    ), "median_sample_spacing_m is infinite"
                    attribute_dict["median_sample_spacing_m"] = median_sample_spacing_m
                except Exception as e:
                    logger.warning(
                        "Unable to compute median sample spacing : {}".format(e)
                    )
                    try:
                        del (
                            self.netcdf_dataset.median_sample_spacing_m
            ...
            logger.debug(
                "Finished setting global geometric metadata attributes in netCDF line dataset"
            )
        except:
            logger.error(
                "Unable to set geometric metadata attributes in netCDF line dataset"
            )
            raise
    ...
    def fix_missing_coordinates(self):
        ...
        def interpolate(last_good_coord_index, next_good_coord_index, bad_point_count):
            ...
            def set_missing_ordinates(
                ...
                assert np.all(
                    np.any(np.isnan(self.xycoords[update_slice]), axis=1)
                ), "Unable to overwrite good coordinates"
                self.xycoords[update_slice] = (
                    interpolated_point_array  # TODO: Check this - it could be dangerous setting a property
                )
                self.netcdf_dataset.variables["longitude"][update_slice] = (
                    interpolated_point_array[:, 0]
                )
                self.netcdf_dataset.variables["latitude"][update_slice] = (
                    interpolated_point_array[:, 1]
                )

                # Set coordinate flag indices
                self.netcdf_dataset.variables["coordinate_flag_index"][
                    update_slice
                ] = flag_index_value

            logger.debug(
                "last_good_coord_index = {}, next_good_coord_index = {}, bad_point_count = {}".format(
                    last_good_coord_index, next_good_coord_index, bad_point_count
                )
            )
            if last_good_coord_index is not None:
                logger.debug(
                    "self.xycoords[self.xycoords[last_good_coord_index] = {}".format(
                        self.xycoords[last_good_coord_index]
                    )
                )
            if next_good_coord_index is not None:
                logger.debug(
                    "self.xycoords[self.xycoords[next_good_coord_index] = {}".format(
                        self.xycoords[next_good_coord_index]
                    )
                )

            interpolated_point_array = np.ndarray(
                shape=(bad_point_count, 2), dtype=self.xycoords.dtype
            )

            if (
                last_good_coord_index is not None and next_good_coord_index is not None
            ):  # Interpolation between points required
                ...
                set_missing_ordinates(
                    self,
                    interpolated_point_array,
                    start_index=last_good_coord_index + 1,
                    flag_index_value=INTERPOLATED_COORDINATE_FLAG,
                )

            elif (
                last_good_coord_index is None and next_good_coord_index is not None
            ):  # Extrapolation to start of line required
                known_coords = self.xycoords[
                    next_good_coord_index : next_good_coord_index + 2
                ]
                assert np.all(
                    ~np.isnan(known_coords)
                ), "Unable to extrapolate to a single point"
                delta = known_coords[0] - known_coords[1]  # Note negative delta
                for interpolated_point_index in range(bad_point_count, 0, -1):
                    interpolated_point_array[interpolated_point_index - 1] = (
                        known_coords[0]
                        + (bad_point_count - interpolated_point_index + 1) * delta
                    )
                set_missing_ordinates(
                    self,
                    interpolated_point_array,
                ...
                last_good_coord_index is not None and next_good_coord_index is None
            ):  # Extrapolation from end of line required
                known_coords = self.xycoords[
                    last_good_coord_index - 1 : last_good_coord_index + 1
                ]
                assert np.all(
                    ~np.isnan(known_coords)
                ), "Unable to extrapolate to a single point"
        ...
        try:
            ...
            try:
                self.netcdf_dataset.createDimension(
                    "coordinate_flag", len(COORDINATE_FLAG_LIST)
                )
                ...
                coordinate_flag_index_variable = self.netcdf_dataset.createVariable(
                    ...
                    ["point"],
                    chunksizes=(
                        [min(DEFAULT_CHUNK_SPEC["point"], self.point_count)]
            ...
            bad_coord_start_mask = np.ones(
                shape=bad_coord_mask.shape, dtype=bool
            )  # Make first element True
            bad_coord_start_mask[1:] = ~bad_coord_mask[
                0:-1
            ]  # Copy all cells shifted rightward
            bad_coord_start_mask[:] = np.logical_and(
                bad_coord_mask, bad_coord_start_mask
            )
            bad_coord_start_indices = np.where(bad_coord_start_mask)[0]

            bad_coord_end_mask = np.ones(
                shape=bad_coord_mask.shape, dtype=bool
            )  # Make last element True
            bad_coord_end_mask[0:-1] = ~bad_coord_mask[
                1:
            ]  # Copy inverted cells shifted leftward
            bad_coord_end_mask[:] = np.logical_and(bad_coord_mask, bad_coord_end_mask)
            bad_coord_end_indices = np.where(bad_coord_end_mask)[0]

            assert (
                bad_coord_start_indices.shape == bad_coord_end_indices.shape
            ), "Every bad coordinate range needs a start and end index"
            logger.debug(
                "{} invalid coordinates found in {} blocks".format(
                    np.count_nonzero(bad_coord_mask), bad_coord_start_indices.shape[0]
                )
            )

            coordinate_flag_index_variable[:] = (
                OBSERVED_COORDINATE_FLAG  # Default to "Observed"
            )
            coordinate_flag_index_variable[bad_coord_mask] = (
                INVALID_COORDINATE_FLAG  # Set bad coordinates to "Invalid"
            )
            ...
            for bad_range_index in range(len(bad_coord_start_indices)):
                ...
                try:
                    ...
                    bad_line_start_index = line_start_indices[
                        np.where(
                            np.logical_and(
                                (line_start_indices >= bad_coord_start_index),
                                (line_start_indices <= bad_coord_end_index),
                            )
                        )[0][0]
                    ]
                except IndexError:
                ...
                try:
                    ...
                    bad_line_end_index = line_end_indices[
                        np.where(
                            np.logical_and(
                                (line_end_indices >= bad_coord_start_index),
                                (line_end_indices <= bad_coord_end_index),
                            )
                        )[0][-1]
                    ]
                except IndexError:
                ...
                logger.debug(
                    "Extended missing coordinate range:\n{}".format(
                        self.xycoords[
                            bad_coord_start_index - 1 : bad_coord_end_index + 2
                        ]
                    )
                )

                logger.debug(
                    "Extended Line numbers for missing points:\n{}".format(
                        self.netcdf_dataset.variables["line"][:][
                            (
                                self.netcdf_dataset.variables["line_index"][
                                    bad_coord_start_index - 1 : bad_coord_end_index + 2
                                ]
                            )
                        ]
                ...
                if (
                    bad_line_end_index is not None
                ...
                if (
                    bad_line_start_index is not None
                ):  # Coordinates need to be extrapolated to start of line
                ...
                if (
                    bad_line_end_index is not None
                ):  # Coordinates need to be extrapolated from end of line
                ...
                if (
                    bad_line_start_index is None and bad_line_end_index is None
                ):  # Coordinates need to be interpolated between good points within a line
                    logger.debug(
                        "Coordinates need to be interpolated between good points within a line"
                    )
                    interpolate(
            ...
            logger.info(
                "Finished interpolating and extrapolating missing coordinates in netCDF line dataset"
            )

            for flag_index in range(len(COORDINATE_FLAG_LIST)):
                logger.debug(
                    "{} coordinate count = {}".format(
                        COORDINATE_FLAG_LIST[flag_index],
                        np.count_nonzero(
            ...
            logger.error(
                "Unable to interpolate and extrapolate missing coordinates in netCDF line dataset"
            )
            raise
...
if __name__ == "__main__":
    ...
    nclu = NetCDFLineUtils(
        "C:\\Users\\alex\\Documents\\GADDS2\\P544MAG.nc", debug=False
    )
    print(
        "{} points in {} lines".format(
            nclu.point_count, nclu.netcdf_dataset.dimensions["line"].size
        )
    )
