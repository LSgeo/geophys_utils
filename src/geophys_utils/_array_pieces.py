...
def array_pieces(ndarray, max_bytes=None, overlap=0):
    ...
    array_bytes = ndarray.dtype.itemsize * reduce(lambda x, y: x * y, array_shape)
    ...
    if array_bytes > max_bytes:  # Multiple pieces required
        ...
        axis_divisions = int(
            math.ceil(
                math.pow(
                    math.ceil(array_bytes / float(max_bytes)), 1.0 / array_dimensions
                )
            )
        )
        ...
        chunking = [
            (
                chunking[index]
                if chunking[index] < array_shape[index] // axis_divisions
                else 1
            )
        ...
        piece_shape = [
            array_shape[index] // axis_divisions // chunking[index] * chunking[index]
            for index in range(array_dimensions)
        ]

        # Determine total number of pieces in each axis
        axis_pieces = [
            int(math.ceil(float(array_shape[index]) // piece_shape[index]))
            for index in range(array_dimensions)
        ]
        ...
        for piece_indices in itertools.product(
            *[
                range(axis_pieces[dimension_index])
                for dimension_index in range(array_dimensions)
            ]
        ):
            # Compute base start indices with no overlap
            start_indices = [
                piece_indices[dimension_index] * piece_shape[dimension_index]
                for dimension_index in range(array_dimensions)
            ]
            ...
            end_indices = [
                min(
                    start_indices[dimension_index]
                    + piece_shape[dimension_index]
                    + overlap,
            ...
            start_indices = [
                max(0, start_indices[dimension_index] - overlap)
                for dimension_index in range(array_dimensions)
            ]

            array_slices = tuple(
                [
                    slice(start_indices[dimension_index], end_indices[dimension_index])
                    for dimension_index in range(array_dimensions)
                ]
            )
...
def main():
    ...
    for piece_array, array_offset in array_pieces(data_variable, overlap=0):
        piece_count += 1
        piece_bytes = data_variable.dtype.itemsize * reduce(
            lambda x, y: x * y, piece_array.shape
        )
        print(
            "piece_array.shape = %s, array_offset = %s, piece_bytes = %d".format(
                piece_array.shape, array_offset, piece_bytes
            )
        )
