...
class TestDataStats(unittest.TestCase):
    ...
    def test_data_stats(self):
        ...
        TestDataStats.EXPECTED_RESULT["nc_path"] = os.path.join(
            os.path.dirname(__file__), TestDataStats.NC_PATH
        )
        data_stats = DataStats(
            TestDataStats.EXPECTED_RESULT["nc_path"], max_bytes=TestDataStats.MAX_BYTES
        )
        for key in sorted(TestDataStats.EXPECTED_RESULT.keys()):
            try:
                error = data_stats.value(key) - TestDataStats.EXPECTED_RESULT[key]
                assert (
                    error <= TestDataStats.MAX_ERROR
                ), "Incorrect numeric value for {}\nExpected {}, got {}.".format(
                    key, TestDataStats.EXPECTED_RESULT[key], data_stats.value(key)
                )
            except TypeError:
                assert data_stats.value(key) == TestDataStats.EXPECTED_RESULT[key], (
                    "Incorrect value for %s" % key
                )
