...
TEST_MANY_COORDS = np.array(
    [[148.484, -35.352], [148.328, -35.428], [148.436, -35.744], [148.300, -35.436]]
)
TEST_INDICES = [1, 1]
...
class TestNetCDFGridUtilsFunctions1(unittest.TestCase):
    ...
    def test_get_indices_from_coords(self):
        print(
            "Testing get_indices_from_coords function with single coordinate {}".format(
                TEST_COORDS
            )
        )
        ...
        ).all, "Incorrect indices: {} instead of {}".format(indices, TEST_INDICES)

        print(
            "Testing get_indices_from_coords function with multi coordinates {}".format(
                TEST_MULTI_COORDS
            )
        )
        multi_indices = netcdf_grid_utils.get_indices_from_coords(TEST_MULTI_COORDS)
        assert (
            multi_indices == np.array(TEST_MULTI_INDICES)
        ).all, "Incorrect indices: {} instead of {}".format(
    ...
    def test_get_fractional_indices_from_coords(self):
        ...
        for ordinate_index in range(len(indices)):
            assert (
                round(indices[ordinate_index], 6)
                == TEST_FRACTIONAL_INDICES[ordinate_index]
            ), "Fractional index incorrect"

    def test_get_value_at_coords(self):
        print(
            "Testing get_value_at_coords function with single coordinate {}".format(
                TEST_COORDS
            )
        )
        ...
        assert (
            value[0] == TEST_VALUE
        ), "Incorrect retrieved value: {} instead of {}".format(value.data, TEST_VALUE)

        print(
            "Testing get_value_at_coords function with multiple coordinates {}".format(
                TEST_MULTI_COORDS
            )
        )
        multi_values = netcdf_grid_utils.get_value_at_coords(TEST_MULTI_COORDS)
        assert (
            np.abs(np.array(multi_values) - np.array(TEST_MULTI_VALUES)) < MAX_ERROR
        ).all(), "Incorrect retrieved value: {} instead of {}".format(
            multi_values, TEST_MULTI_VALUES
        )

        print(
            "Testing get_value_at_coords with long coordinate list {}".format(
                TEST_MANY_COORDS
            )
        )
        many_values = netcdf_grid_utils.get_value_at_coords(TEST_MANY_COORDS)
        assert (
            np.abs(np.array(many_values) - np.array(TEST_MANY_VALS)) < MAX_ERROR
        ).all(), "Incorrect retrieved value: {} instead of {}".format(
            many_values, TEST_MANY_VALS
        )

        print(
            "Testing get_value_at_coords with long coordinate list {} and request size {} bytes".format(
                TEST_MANY_COORDS, MAX_BYTES_SM
            )
        )
        many_values = netcdf_grid_utils.get_value_at_coords(
            TEST_MANY_COORDS, max_bytes=MAX_BYTES_SM
        )
        assert (
            np.abs(np.array(many_values) - np.array(TEST_MANY_VALS)) < MAX_ERROR
        ).all(), "Incorrect retrieved value: {} instead of {}".format(
        ...
        many_values = netcdf_grid_utils.get_value_at_coords(
            TEST_MANY_COORDS, max_bytes=MAX_BYTES_LG
        )
        assert (
            np.abs(np.array(many_values) - np.array(TEST_MANY_VALS)) < MAX_ERROR
        ).all(), "Incorrect retrieved value: {} instead of {}".format(
    ...
    def test_get_interpolated_value_at_coords(self):
        ...
        interpolated_value = netcdf_grid_utils.get_interpolated_value_at_coords(
            TEST_COORDS
        )
        assert (
            interpolated_value == TEST_INTERPOLATED_VALUE
        ), "Incorrect interpolated value retrieved"
