...
class TestCRSUtils(unittest.TestCase):
    ...
    UTM_COORDS = [
        696382.56321712,
        6090881.85849329,
    ]  # (696382.5632178178, 6090881.858493158)
    UTM_COORD_ARRAY = [
        (696382.5632171195, 6090881.858493287),
        (773798.0963396085, 6122843.308355326),
    ]
    ...
    def test_get_coordinate_transformation(self):
        ...
        coordinate_transformation = get_coordinate_transformation(
            TestCRSUtils.EPSG4326_WKT, TestCRSUtils.EPSG3577_WKT
        )
        assert coordinate_transformation is not None
        assert type(coordinate_transformation) == CoordinateTransformation

        coordinate_transformation = get_coordinate_transformation(
            TestCRSUtils.EPSG4326_WKT, TestCRSUtils.GDA2020_WKT
        )
        assert coordinate_transformation is not None
        assert type(coordinate_transformation) == CoordinateTransformation

        coordinate_transformation = get_coordinate_transformation(
            TestCRSUtils.GDA2020_WKT, TestCRSUtils.EPSG4326_WKT
        )
        assert coordinate_transformation is not None
        assert type(coordinate_transformation) == CoordinateTransformation

        coordinate_transformation = get_coordinate_transformation(
            TestCRSUtils.EPSG4326_EPSG, TestCRSUtils.EPSG3577_EPSG
        )
        assert coordinate_transformation is not None
        assert type(coordinate_transformation) == CoordinateTransformation

        coordinate_transformation = get_coordinate_transformation(
            TestCRSUtils.EPSG4326_WKT, TestCRSUtils.EPSG4326_WKT
        )
        assert (
            coordinate_transformation is None
        ), "Null transformation should return None"

    def test_get_utm_wkt(self):
        print("Testing get_utm_wkt function")
        utm_wkt = get_utm_wkt(TestCRSUtils.EPSG4326_COORDS, TestCRSUtils.EPSG4326_EPSG)
        utm_wkt = re.sub(",\s+", ",", re.sub("\s+", " ", utm_wkt))
        expected_wkt = re.sub(",\s+", ",", re.sub("\s+", " ", TestCRSUtils.UTM_WKT))
        assert utm_wkt == expected_wkt, "Incorrect UTM CRS: {} instead of {}".format(
            utm_wkt, expected_wkt
        )

    def test_transform_coords(self):
        print(
            "Testing transform_coords function with single coordinate{}".format(
                TestCRSUtils.EPSG4326_COORDS
            )
        )
        utm_coords = transform_coords(
            TestCRSUtils.EPSG4326_COORDS,
            TestCRSUtils.EPSG4326_WKT,
            TestCRSUtils.UTM_WKT,
        )
        assert (
            abs(utm_coords - np.array(TestCRSUtils.UTM_COORDS)) < COORD_ERROR_MARGIN
        ).all(), "Incorrect UTM coordinates: {} instead of {}".format(
            utm_coords, np.array(TestCRSUtils.UTM_COORDS)
        )

        print(
            "Testing transform_coords function with multi coordinate {}".format(
                TestCRSUtils.EPSG4326_COORD_ARRAY
            )
        )
        utm_coord_array = transform_coords(
            TestCRSUtils.EPSG4326_COORD_ARRAY,
            TestCRSUtils.EPSG4326_WKT,
            TestCRSUtils.UTM_WKT,
        )
        assert (
            abs(utm_coord_array - TestCRSUtils.UTM_COORD_ARRAY) < COORD_ERROR_MARGIN
        ).all(), "Incorrect UTM coordinates: {} instead of {}".format(
