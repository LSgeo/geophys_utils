...
class TestNetCDFPointUtilsConstructor(unittest.TestCase):
    ...
    def test_netcdf_point_utils_constructor(self):
        ...
        assert (
            nc_dataset.title == NC_TITLE
        ), 'Invalid dataset title: "{}" != "{}"'.format(nc_dataset.title, NC_TITLE)
...
class TestNetCDFPointUtilsFunctions1(unittest.TestCase):
    ...
    def test_get_spatial_mask(self):
        ...
        # print(spatial_mask)
        assert (
            np.count_nonzero(spatial_mask) == SPATIAL_MASK_COUNT
        ), f"Unexpected spatial mask count {np.count_nonzero(spatial_mask)} != {SPATIAL_MASK_COUNT}"
...
class TestNetCDFPointUtilsGridFunctions(unittest.TestCase):
    ...
    def test_grid_points(self):
        ...
        grids, crs, geotransform = netcdf_point_utils.grid_points(
            grid_resolution=GRID_RESOLUTION, variables=VARIABLES, point_step=POINT_STEP
        )

        assert (crs, geotransform, grids.shape) == TEST_GRID_RESULTS[
            0
        ], "Invalid grid results: {} != {}".format(
...
def test_suite():
    ...
    test_classes = [
        TestNetCDFPointUtilsConstructor,
