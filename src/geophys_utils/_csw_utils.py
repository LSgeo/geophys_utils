...
class CSWUtils(object):
    ...
    def __init__(
        ...
        settings_path = settings_path or os.path.join(
            package_dir, "csw_utils_settings.yml"
        )
    ...
    def get_date_filter(
        self, start_datetime=None, stop_datetime=None, constraint="overlaps"
    ):
        ...
        if constraint == "overlaps":
            start_filter = fes.PropertyIsLessThanOrEqualTo(
                propertyname="ows100:TempExtent_begin", literal=stop_date_string
            )
            stop_filter = fes.PropertyIsGreaterThanOrEqualTo(
                propertyname="ows100:TempExtent_end", literal=start_date_string
            )
        elif constraint == "within":
            start_filter = fes.PropertyIsGreaterThanOrEqualTo(
                propertyname="ows100:TempExtent_begin", literal=start_date_string
            )
            stop_filter = fes.PropertyIsLessThanOrEqualTo(
                propertyname="ows100:TempExtent_end", literal=stop_date_string
            )
                    ...
                    fes.Or(
                        [
                            ...
                            for word_variant in set(
                                [word, word.upper(), word.lower(), word.title()]
                            )
                        ]
                    )
                )
    ...
    def get_csw_records(
        ...
        max_total_records = (
            max_total_records or self.settings["DEFAULT_MAXTOTALRECORDS"]
        )
        ...
        for csw in self.csw_list:
            ...
            while startposition and record_count < max_total_records:
                ...
                if not csw.results[
                    "returned"
                ]:  # Don't go around again for another query - should be the end
                    break

                for uuid in [
                    uuid for uuid in csw.records.keys() if uuid not in uuid_list
                ]:
                    ...
                    uuid_list.append(
                        uuid
                    )  # Remember UUID to avoid returning duplicates
                    ...
                    if record.bbox:
                        ...
                        record_dict["bbox_crs"] = (
                            str(record.bbox.crs) or "urn:ogc:def:crs:EPSG::4283"
                        )  # Default to GDA94
                        ...
                        if (
                            record_dict["bbox_crs"][-1] == ")"
                        ):  # Hack for invalid CRS code "urn:ogc:def:crs:EPSG::GDA94 (EPSG:4283)"
                        ...
                        record_dict["bbox"] = [
                            min(record_dict["bbox"][::2]),
                            min(record_dict["bbox"][1::2]),
                            max(record_dict["bbox"][::2]),
                            max(record_dict["bbox"][1::2]),
                        ]
                    ...
                    if get_layers:
                        for distribution_info in [
                            ...
                            try:
                                wms = WebMapService(
                                    distribution_info["url"], version="1.1.1"
                                )
                        ...
                        for distribution_info in [
                            ...
                            try:
                                wcs = WebCoverageService(
                                    distribution_info["url"], version="1.0.0"
                                )
                    ...
                    yield record_dict
                    logger.debug(
                        '{} distribution(s) found for "{}"'.format(
                            len(distribution_info_list), title
                        )
                    )

                    if (
                        record_count >= max_total_records
                    ):  # Don't go around again for another query - maximum retrieved
                        raise Exception(
                            "Maximum number of records retrieved ({})".format(
                                max_total_records
                            )
                        )
    ...
    def query_csw(
        ...
        get_layers = (
            self.settings["DEFAULT_GET_LAYERS"] if get_layers is None else get_layers
        )
        ...
        if alt_identifier_list:
            if len(alt_identifier_list) == 1:
                fes_filter_list += [
                    fes.PropertyIsLike(
                        propertyname="AlternateIdentifier",
        ...
        if anytext_list:
            fes_filter_list += [
                fes.PropertyIsLike(
                    propertyname="anyText", literal=phrase, matchCase=False
                )
                for phrase in anytext_list
            ]
        ...
        if titleword_list:
            fes_filter_list += [
                fes.PropertyIsLike(
                    propertyname="title", literal=titleword, matchCase=False
                )
        ...
        return self.get_csw_records(
            fes_filter_list, max_total_records=max_total_records, get_layers=get_layers
        )
    ...
    def flatten_distribution_dict(self, record_dict, distribution_dict):
        ...
        """
        dataset_distribution_dict = copy.copy(
            record_dict
        )  # Create shallow copy of record dict

        # Delete list of all distributions from copy of record dict
        del dataset_distribution_dict["distributions"]

        # Convert lists to strings
        dataset_distribution_dict["identifiers"] = ", ".join(
            sorted(dataset_distribution_dict["identifiers"])
        )

        dataset_distribution_dict["keywords"] = ", ".join(
            sorted(dataset_distribution_dict["keywords"])
        )

        bbox = dataset_distribution_dict.get("bbox")  # Retrieve bounding box
        if bbox:
            dataset_distribution_dict["bbox"] = ", ".join(
                str(ordinate) for ordinate in bbox
            )
        else:
        ...
        if distribution_dict:
            ...
            dataset_distribution_dict["url"] = re.sub(
                "^file://", "", dataset_distribution_dict["url"]
            )

            if "layers" in dataset_distribution_dict.keys():
                dataset_distribution_dict["layers"] = ", ".join(
                    dataset_distribution_dict["layers"]
                )
    ...
    def partial_string_match(self, superstring, partial_string_list):
        ...
        for search_string in partial_string_list:
            if not (search_string or superstring) or (  # Both empty strings
                search_string and search_string in superstring
            ):  # Substring
                return True
        return False
    ...
    def get_distributions(self, distribution_protocols, dataset_dict_generator):
        ...
        if type(distribution_protocols) == str:
            search_protocol_list = [
                distribution_protocol.lower()
                for distribution_protocol in self.list_from_comma_separated_string(
            ...
            if record_dict["distributions"]:
                for distribution_dict in record_dict["distributions"]:
                    ...
                    if self.partial_string_match(
                        distribution_dict["protocol"].lower(), search_protocol_list
                    ):
                        yield self.flatten_distribution_dict(
                            record_dict, distribution_dict
                        )
                ...
                if "file" in file_distribution.get("protocol", "").lower():
                    ...
                    try:
                        file_distribution["url"] = match.group(
                            2
                        )  # Ignore any leading "file://"
                        if os.path.isfile(file_distribution["url"]) and netCDF4.Dataset(
                            file_distribution["url"]
                        ):  # Test for valid netCDF file
                            # logger.debug('file found')
                            distribution_dict = file_distribution
                            break
                    except:
                        logger.warning(
                            "Unable to open netCDF file {}".format(
                                file_distribution["url"]
                            )
                        )
                ...
                for opendap_distribution in record_dict["distributions"]:
                    if "opendap" in opendap_distribution["protocol"].lower():
                        ...
                        try:
                            ...
                            ):  # Test for valid OPeNDAP endpoint
                                distribution_dict = opendap_distribution
                                break
                        except:
                            logger.warning(
                                "Unable to open OPeNDAP URL {}".format(
                                    opendap_distribution["url"]
                                )
                            )
