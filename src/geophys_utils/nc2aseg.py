...
assert not MAX_FIELD_WIDTH or all(
    [
        format_specification["width"] <= MAX_FIELD_WIDTH
        for format_specification in ASEG_GDF_FORMAT.values()
    ]
), "Invalid MAX_FIELD_WIDTH {}".format(MAX_FIELD_WIDTH)
...
class RowValueCache(object):
    ...
    def read_points(self, start_index, end_index, point_mask=None):
        ...
        for field_name in self.field_definitions.keys():
            self.cache[field_name] = self.nc2aseggdf.get_data_values(
                field_name, slice(start_index, end_index)
            )
            netcdf_fill_value = self.nc2aseggdf.get_netcdf_fill_value(field_name)
            if netcdf_fill_value is not None:
                ascii_null_value = self.field_definitions[field_name]["format"].get(
                    "null"
                )
...
class NC2ASEGGDF2(object):
    ...
    def __init__(
        ...
        def get_preferred_aseggdf2_integer_width_and_null(data_vals):
            ...
            max_abs_value = np.nanmax(
                np.abs(np.array([min_value, max_value]).astype("float64"))
            )
        ...
        def build_field_definitions():
            ...
            for variable_name, variable in self.netcdf_dataset.variables.items():
                ...
                if any(
                    [
                        re.match(exclude_name_regex, variable_name, re.IGNORECASE)
                ...
                if len(variable.dimensions) == 1 and variable.dimensions != (
                    ...
                    try:
                        ...
                        logger.debug(
                            "Found index variable {} for lookup variable {}".format(
                                index_variable.name, variable_name
                            )
                        )
                    except:
                        logger.debug(
                            "Index variable not found for lookup variable {}".format(
                                variable_name
                            )
                        )
                    ...
                    logger.debug(
                        "Found point-wise array data variable {}".format(variable_name)
                    )
                    variables = [
                        variable
                    ]  # Not an index variable - just use primary variable values

                elif (
                    not len(variable.dimensions)
                    and variable_name != self.ncpu.crs_variable.name
                ):
                    logger.debug(
                        "Found point-wise scalar data variable {}".format(variable_name)
                    )
                    variables = [
                        variable
                    ]  # Scalar variable - broadcast out to all points

                else:
                    logger.debug(
                        "Unable to deal with variable {} - ignoring".format(
                            variable_name
                        )
                    )
                    continue
                ...
                if not format_dict:  # Unrecognised format. Treat as string
                    width = (
                        max([len(str(element).strip()) for element in variables[-1][:]])
                        + 1
                    )
                ...
                try:
                    column_count = reduce(
                        lambda x, y: x * y, variable.shape[1:]
                    )  # This will work for (2+)D, even if we only support 1D or 2D
                except:  # Scalar or 1D
                    column_count = 1
                ...
                if TRUNCATE_VARIABLE_NAMES:
                    ...
                    while field_name in [
                        ...
                        field_name_count += 1
                        field_name = field_name[: -len(str(field_name_count))] + str(
                            field_name_count
                        )
                else:
                    field_name = re.sub(
                        "\W+", "_", variable_name
                    )  # Sanitisation shouldn't be necessary, but we'll do it anyway
                ...
                if ADJUST_INTEGER_FIELD_WIDTH and "int" in str(
                    ...
                    logger.debug(
                        "\tChecking values to adjust integer field width for variable {}".format(
                            variable_name
                        )
                    )
                    width, nullvalue = get_preferred_aseggdf2_integer_width_and_null(
                        data_vals
                    )

                    if (format_dict["width"] != width) or (
        ...
        self.netcdf_dataset.set_auto_mask(
            False
        ...
        self.line_report_increment = (
            10.0 ** int(log10(self.ncpu.point_count / 50))
        ) / 2.0
        ...
        if len(variables) == 1:  # Single variable => no lookup
            if len(variables[0].dimensions):  # Array
                ...
                data = np.array(
                    ...
                    * (
                        (
                            (point_slice.stop or self.total_points)
                            - (point_slice.start or 0)
                        )
            ...
            if np.any(variables[0][:] == mask_value_index_var):
                logger.debug(
                    "Variable '{}' contains one or more masked values. Converting masked value/s to {}".format(
                ...
                data = [None] * (
                    lookup_len
                )  # create a list of the required size to fill in with correct values
                while i < lookup_len:
            ...
            # values
                ...
                data = variables[1][:][
                    variables[0][:][point_slice]
                ]  # Use first array to index second one
        else:
            raise BaseException(
                "Unable to resolve chained lookups (yet): {}".format(
    ...
    def create_dfn_line(
        ...
        if defn is None:
            self.defn += (
                1  # Increment last DEFN value (initialised to 0 in constructor)
            )
        else:
            self.defn = defn

        line = "DEFN {defn} ST={st},RT={rt}; {name}".format(
            defn=self.defn,
            st=st,
            rt=rt,
            name=name,
    ...
    def dfn_line_generator(self):
            ...
            for field_name, field_definition in self.field_definitions.items():
                ...
                if null_value is not None:
                    optional_attribute_list.append(
                        "NULL="
                        + field_definition["format"]["python_format"]
                        .format(null_value)
                        .strip()
                    )

                long_name = field_definition["attributes"].get("long_name") or re.sub(
                    "(\W|_)+", " ", field_definition["variable_name"]
                )
                if long_name:
                    optional_attribute_list.append(
                        "NAME={long_name}".format(long_name=long_name)
                    )

                # Include any variable attributes which match regexes in INCLUDE_VARIABLE_ATTRIBUTE_REGEXES
                for attribute_name, attribute_value in field_definition[
                    "attributes"
                ].items():
                    if any(
                        [
                            re.match(
                                variable_attribute_regex, attribute_name, re.IGNORECASE
                            )
                ...
                ):  # Need to pre-pend number of columns to format string
                    aseg_gdf_format = "{}{}".format(
                        field_definition["columns"], aseg_gdf_format
                    )
        ...
        def proj_defns_generator():
            ...
            projcs = self.spatial_ref.GetAttrValue(
                "projcs"
            )  # e.g. 'UTM Zone 54, Southern Hemisphere'
            ...
            if self.spatial_ref.IsProjected():
                if projcs.startswith(geogcs):
                    ...
                    projection_name = (
                        geogcs + " / " + re.sub("[\:\,\=]+", "", projcs)
                    )  # e.g. 'GDA94 / UTM Zone 54, Southern Hemisphere'
                projection_method = self.spatial_ref.GetAttrValue("projection").replace(
                    "_", " "
                )
            ...
            yield self.create_dfn_line(rt="PROJ", name="RT", aseg_gdf_format="A4")
            ...
            yield self.create_dfn_line(
                ...
                definition="UNIT={unit}, NAME={major_axis}, Major axis".format(
                    unit="m", major_axis=major_axis
                ),
            )
            ...
            # Write 'END DEFN'
            yield self.create_dfn_line(rt="PROJ", name="END DEFN", aseg_gdf_format="")
    ...
    def create_dat_file(
        ...
        def chunk_buffer_generator(
            ...
            def chunk_line_generator(
                ...
                row_value_cache.read_points(
                    start_index, end_index, point_mask=point_mask
                )

                logger.debug(
                    "Preparing ASEG-GDF lines for rows {:n} - {:n}".format(
                ...
                for row_value_list in row_value_cache.chunk_row_data_generator():
                    ...
                    yield "".join(
                        [
                            " "
                            + python_format_list[value_index].format(
                                row_value_list[value_index]
                            )[1 - MAX_FIELD_WIDTH : :]
                            for value_index in range(len(python_format_list))
                        ]
                    )  # .lstrip() # lstrip if we want to discard leading spaces from line

            # Process all chunks
            point_count = 0
            for chunk_index in range(self.total_points // cache_chunk_rows + 1):
                chunk_line_list = []
                for line in chunk_line_generator(
                    row_value_cache,
                    python_format_list,
                    start_index=chunk_index * cache_chunk_rows,
                    end_index=min(
                        (chunk_index + 1) * cache_chunk_rows, self.total_points
                    ),
                    ...
                    if not (point_count % self.line_report_increment):
                        self.info_output(
                            "{:n} / {:n} ASEG-GDF2 rows converted to text".format(
                    ...
                    if (
                        self.debug
                        and DEBUG_POINT_LIMIT
                        and (point_count >= DEBUG_POINT_LIMIT)
                    ):  # Don't process more lines
                        break
                ...
                if (
                    ...
                    logger.warning(
                        "WARNING: Output limited to {:n} points in debug mode".format(
                            DEBUG_POINT_LIMIT
                        )
                    )
                    break
        ...
        row_value_cache = RowValueCache(
            self
        )  # Create cache for multiple chunks of data
        ...
        if zipstream_zipfile:
            ...
            zipstream_zipfile.write_iter(
                ...
                buffer_size=self.ncpu.point_count
                * LINE_BUFFER_SIZE,  # Need this to force 64-bit zip
            )
        else:  # No zip
            # Create, write and close .dat file
            dat_out_file = open(dat_out_path, mode="w")
            for chunk_buffer in chunk_buffer_generator(
                row_value_cache, python_format_list, cache_chunk_rows, point_mask
            ):
    ...
    def create_des_file(self, des_out_path, zipstream_zipfile=None):
        ...
        def des_line_generator(encoding=None):
            ...
            global_attributes_dict["ASEG_GDF2"] = (
                "Generated at {} from {} using nc2aseg.py".format(
                    datetime.now().isoformat(), os.path.basename(self.netcdf_path)
                )
            )
            ...
            for key in sorted(global_attributes_dict.keys()):
                ...
                key_string = (" {{:<{}s}} : ".format(max_key_length)).format(
                    key
    ...
    def convert2aseg_gdf(
        ...
        self.dat_out_path = (
            dat_out_path or os.path.splitext(self.netcdf_dataset.filepath())[0] + ".dat"
        )
        ...
        if zip_out_path:
            ...
            zipstream_zipfile.comment = (
                "ASEG-GDF2 files generated at {} from {}".format(
                    datetime.now().isoformat(), os.path.basename(self.netcdf_path)
                )
        ...
        self.info_output(
            "ASEG-GDF output completed in {}".format(str(elapsed_time).split(".")[0])
        )  # Discard partial seconds
