...
class PostgresDatasetMetadataCache(DatasetMetadataCache):
    ...
    def __init__(
        ...
        self.autocommit = (
            ...
            else PostgresDatasetMetadataCache.DEFAULT_POSTGRES_AUTOCOMMIT
        )
        ...
        logger.debug(
            "Connected to database {}:{}/{} as {}".format(
                self.postgres_host,
                self.postgres_port,
    ...
    def __del__(self):
        ...
        if self.db_connection:
            logger.debug(
                "Disconnecting from database {}:{}/{}".format(
                    self.postgres_host, self.postgres_port, self.postgres_dbname
                )
            )
