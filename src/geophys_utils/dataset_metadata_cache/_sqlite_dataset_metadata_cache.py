...
class SQLiteDatasetMetadataCache(DatasetMetadataCache):
    ...
    def __init__(self, sqlite_path=None, debug=False, force_recreate=False):
        ...
        )  # Ensure that correct directory separator is used

        if (
            self.sqlite_path[0] != os.path.sep and self.sqlite_path[1] != ":"
        ):  # If path is relative
            self.sqlite_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), self.sqlite_path
            )  # DB is located in the same directory as this file
        ...
        try:
            ...
            if not os.path.isfile(self.sqlite_path):
                raise BaseException(
                    "SQLite Database file {} does not exist".format(self.sqlite_path)
                )
            ...
            if self.db_connection:
                logger.debug(
                    "Disconnecting from old database {}".format(self.sqlite_path)
                )
                self.db_connection.close()

            backup_sqlite_path = (
                "_" + re.sub("\W", "-", datetime.now().isoformat())
            ).join(os.path.splitext(self.sqlite_path))
            logger.info(
                "Backing up old database {} to {}".format(
                    self.sqlite_path, backup_sqlite_path
                )
            )
            try:
            ...
            # Create new database
            logger.info(
                "Creating and connecting to fresh database {}".format(self.sqlite_path)
            )
            self.db_connection = sqlite3.connect(self.sqlite_path)
            cursor = self.db_connection.cursor()
            ...
            script_sql = re.sub(
                "--.*?$",
                "",
                re.sub("/\*.*?\*/", "", script_sql, flags=re.RegexFlag.DOTALL),
                flags=re.RegexFlag.MULTILINE,
            )

            ddl_queries = [
                ddl_query.strip() + ";\n"
                for ddl_query in script_sql.split(";")
                if ddl_query.strip()
            ]

            logger.info(
                "Executing DDL script {}".format(
                    SQLiteDatasetMetadataCache.DDL_SQL_FILE
                )
            )
    ...
    def add_dataset(self, dataset):
        ...
        self.add_survey(
            ga_survey_id=dataset.ga_survey_id,
...
"""
        ...
        if cursor.rowcount:
            logger.info(
                'New dataset "{}" inserted into table'.format(dataset.dataset_title)
            )
        else:
            logger.debug(
                'Dataset "{}" already exists in table'.format(dataset.dataset_title)
            )
...
"""
    ...
    def add_keywords(self, dataset_id, keyword_list):
        ...
        for keyword in keyword_list:
            params = {"dataset_id": dataset_id, "keyword_value": keyword}
...
"""
        ...
        insert_dataset_keyword_sql = (
            """insert into dataset_keyword(dataset_id, keyword_id)
select :dataset_id, 
    keyword_id 
    from keyword
...
"""
    ...
    def add_distributions(self, dataset_id, distribution_list):
        ...
        for protocol_value in set(
            [distribution.protocol for distribution in distribution_list]
        ):
...
"""
            ...
            if cursor.rowcount:
                logger.info(
                    'New protocol "{}" inserted into protocol table'.format(
                        protocol_value
                    )
                )
            else:
                logger.debug(
                    'Protocol "{}" already exists in protocol table'.format(
                        protocol_value
                    )
                )
...
"""
            ...
            if cursor.rowcount:
                logger.info(
                    'Distribution "{}" inserted into table'.format(distribution.url)
                )
            else:
                logger.debug(
                    'Distribution "{}" already exists in table'.format(distribution.url)
                )
...
"""
        for keyword_index in range(len(keyword_list)):
            ...
            dataset_search_sql += (
                """inner join (select dataset_id from dataset_keyword
    inner join keyword using(keyword_id)
    where lower(keyword_value) = '"""
                + keyword.lower()
                + """'
...
"""
        ...
        for row in cursor:
            ...
            for datetime_index in [9, 10]:
                try:
                    row[datetime_index] = datetime.strptime(
                        row[datetime_index], "%Y-%m-%d"
                    ).date()
                except TypeError:
                    row[datetime_index] = None
            row_list.append(
                dict(zip(DatasetMetadataCache.dataset_distribution_search_fields, row))
            )
