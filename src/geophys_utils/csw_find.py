...
def main():
    ...
    def quote_delimitedtext(text, delimiter, quote_char='"'):
        ...
        try:
            if delimiter in text or quote_char in text or re.search("\s", text):
                if (
                    delimiter == ","
                ):  # Use double quote to escape quote character for CSV
                    return (
                        quote_char
    ...
    parser.add_argument(
        "-i",
        "--identifiers",
        help="comma separated list of possible metadata identifiers (UUID) for search",
        type=str,
    )
    parser.add_argument(
        "-j",
    ...
    parser.add_argument(
        "-t",
        "--titlewords",
        help="comma-separated list of required titlewords for search",
        type=str,
    )
    parser.add_argument(
        "-a",
        "--anytext",
        help="comma-separated list of required text snippets for search",
        type=str,
    )
    parser.add_argument(
        "-b",
    ...
    parser.add_argument(
        "-d",
        "--delimiter",
        help='field delimiter for output. Defaults to "\t"',
        type=str,
    )
    parser.add_argument(
        "-u",
    ...
    parser.add_argument(
        "-m", "--max_results", help="Maximum number of records to return", type=int
    )
    parser.add_argument(
        "-r",
        "--header_row",
    ...
    parser.add_argument(
        "-y",
        "--types",
        help="comma-separated list of possible record types for search",
        type=str,
    )
    ...
    cswu = CSWUtils(url_list, debug=args.debug)
    ...
    protocol_list = (
        [
            protocol.strip().lower().replace("none", "")
    ...
    field_list = (
        [field.strip().lower() for field in args.fields.split(",")]
        if args.fields
        else None
    ) or cswu.settings["OUTPUT_DEFAULTS"]["DEFAULT_FIELDS"]
    ...
    record_generator = cswu.query_csw(
        identifier_list=args.identifiers,
    ...
    header_row_required = (
        cswu.settings["OUTPUT_DEFAULTS"]["DEFAULT_SHOW_HEADER_ROW"]
    ...
    for distribution in cswu.get_distributions(protocol_list, record_generator):
        ...
        if header_row_required and not header_printed:
            print(
                delimiter.join(
                    [field for field in (field_list or sorted(distribution.keys()))]
                )
            )
            header_printed = True
        ...
        print(
            delimiter.join(
                [
                    quote_delimitedtext(
                        unidecode(distribution.get(field) or ""), delimiter
                    )
