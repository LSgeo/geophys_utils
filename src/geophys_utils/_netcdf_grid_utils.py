...
MAX_CELLS_TO_COMPUTE_SHAPE = (
    400000000  # Set limit for grids of approximately 20,000 x 20,000
)
SHAPE_BUFFER_DISTANCE = (
    None  # Distance to buffer (kerf) shape out then in again (in degrees)
)
...
class NetCDFGridUtils(NetCDFUtils):
    ...
    def __init__(self, netcdf_dataset, debug=False):
        ...
        def set_nominal_pixel_sizes():
            ...
            centre_pixel_coords = [
                [
                    self.dimension_arrays[dim_index][centre_pixel_indices[dim_index]]
                    for dim_index in range(2)
                ],
                [
                    self.dimension_arrays[dim_index][
                        centre_pixel_indices[dim_index] + 1
                    ]
                    for dim_index in range(2)
                ],
            ]
            ...
            self.y_inverted = (
                self.y_variable[-1] < self.y_variable[0]
            ).item()  # Should not have to deal with null values
            ...
            self.nominal_pixel_metres = [
                round(
                    abs(
                        centre_pixel_utm_coords[1][dim_index]
                        - centre_pixel_utm_coords[0][dim_index]
                    ),
                    8,
                )
                for dim_index in range(2)
            ...
            self.nominal_pixel_degrees = [
                round(
                    abs(
                        centre_pixel_wgs84_coords[1][dim_index]
                        - centre_pixel_wgs84_coords[0][dim_index]
                    ),
                    8,
                )
                for dim_index in range(2)
        ...
        def get_default_sample_metres():
            ...
            return round(
                math.pow(
                    10.0,
                    math.floor(log_10_avg_pixel_metres)
                    + (
                        log_10_5
                        if ((log_10_avg_pixel_metres % 1.0) < log_10_5)
                        else 1.0
                    ),
                )
            )
        ...
        assert (
            len(self.data_variable_list) > 0
        ), 'Unable to determine any data variables (must have same dimensions as variable with "grid_mapping" attribute)'
        ...
        self.YX_order = (
            self.data_variable.dimensions[1] in NetCDFGridUtils.X_DIM_VARIABLE_NAMES
        )

        # Two-element list of dimension varibles.
        self.dimension_arrays = [
            self.netcdf_dataset.variables[dimension_name][:]
        ...
        self.min_extent = tuple(
            [
                min(self.dimension_arrays[dim_index]) - self.pixel_size[dim_index] / 2.0
                for dim_index in range(2)
            ]
        )
        self.max_extent = tuple(
            [
                max(self.dimension_arrays[dim_index]) + self.pixel_size[dim_index] / 2.0
                for dim_index in range(2)
            ]
        )
        ...
        self.native_bbox = [
            [
                self.GeoTransform[0]
                + (x_pixel_offset * self.GeoTransform[1])
                + (y_pixel_offset * self.GeoTransform[2]),
                self.GeoTransform[3]
                + (x_pixel_offset * self.GeoTransform[4])
                + (y_pixel_offset * self.GeoTransform[5]),
            ]
            for x_pixel_offset, y_pixel_offset in [
                [0, self.pixel_count[1]],
        ...
        try:  # Multiple coordinates
            indices = [
                (
                    [
                        np.where(
                            abs(
                                self.dimension_arrays[dim_index] - coordinate[dim_index]
                            )
                            <= (self.pixel_size[dim_index] / 2.0)
                        )[0][0]
                        for dim_index in range(2)
                    ]
                    if not (
                        [
                            True
                            for dim_index in range(2)
                            if coordinate[dim_index] < self.min_extent[dim_index]
                            or coordinate[dim_index] > self.max_extent[dim_index]
                        ]
                    )
                    else None
                )
            ...
            indices = (
                ...
                if not [
                    True
                    for dim_index in range(2)
                    if native_coordinates[dim_index] < self.min_extent[dim_index]
                    or native_coordinates[dim_index] > self.max_extent[dim_index]
                ]
                else None
            )
    ...
    def get_fractional_indices_from_coords(self, coordinates, wkt=None):
        ...
        try:  # Multiple coordinates
            fractional_indices = [
                (
                    [
                        (coordinate[dim_index] - min(self.dimension_arrays[dim_index]))
                        / self.pixel_size[dim_index]
                        for dim_index in range(2)
                    ]
                    if not (
                        [
            ...
            fractional_indices = (
                [
                    (
                        native_coordinates[dim_index]
                        - min(self.dimension_arrays[dim_index])
                    )
                    / self.pixel_size[dim_index]
                    for dim_index in range(2)
                ]
    ...
    def get_value_at_coords(
        ...
        assert (
            len(index_array.shape) == 2 and index_array.shape[1] == 2
        ), "Not an iterable containing index pairs"
        # Boolean mask indicating which index pairs are valid
        mask_array = np.array([(index_pair is not None) for index_pair in indices])
        # Array of values read from variable
        value_array = (
            np.ones(shape=(len(index_array)), dtype=data_variable.dtype) * no_data_value
        )
        # Final result array including no-data for invalid index pairs
        result_array = (
            np.ones(shape=(len(mask_array)), dtype=data_variable.dtype) * no_data_value
        )
        start_index = 0
        while start_index < len(index_array):
            ...
            end_index, bbox = _get_query_params(
                index_array, start_index, data_variable, max_bytes
            )
    ...
    def get_interpolated_value_at_coords(
        ...
        fractional_indices = self.get_fractional_indices_from_coords(coordinates, wkt)
        ...
        try:
            ...
            # Boolean mask indicating which index pairs are valid
            mask_array = np.array(
                [(index_pair is not None) for index_pair in fractional_indices]
            )
            # Array of values read from variable
            value_array = (
                np.ones(shape=(len(index_array)), dtype=data_variable.dtype)
                * no_data_value
            )
            # Final result array including no-data for invalid index pairs
            result_array = (
                np.ones(shape=(len(mask_array)), dtype=data_variable.dtype)
                * no_data_value
            )
            ...
            result_array[
                np.array(
                    self.get_value_at_coords(coordinates, wkt, max_bytes, variable_name)
                )
                == no_data_value
            ] = no_data_value
            ...
            return map_coordinates(
                data_variable,
                np.array([[fractional_indices[0]], [fractional_indices[1]]]),
                cval=no_data_value,
            )
    ...
    def get_convex_hull(self, to_wkt=None):
        ...
        try:
            convex_hull = netcdf2convex_hull(
                self.netcdf_dataset, NetCDFGridUtils.DEFAULT_MAX_BYTES
            )
        except:
            logger.warning(
                "Unable to compute convex hull. Using rectangular bounding box instead."
            )
    ...
    def get_concave_hull(
        ...
        buffer_distance = (
            buffer_distance or max(self.data_variable.shape) // 20
        )  # Tune this to suit overall size of grid
        # buffer_distance = 1000000
        logger.debug("buffer distance: {}".format(buffer_distance))
        offset = (
            offset or 0.9
        )  # Take final shape half a pixel out from centre coordinates to cover pixel edges
        ...
        def discard_internal_polygons(geometry):
            ...
            if type(geometry) == MultiPolygon:
                ...
                logger.debug(
                    "{} internal polygons discarded.".format(
                        len(geometry.geoms) - len(polygon_list)
                    )
                )
                ...
                raise ValueError(
                    "Unexpected type of geometry: {}".format(type(geometry))
                )
                    ...
                    [
                        Polygon(
                            transform_coords(
                                np.array(
                                    [
                                        (affine_transform * pixel_coordinate_pair)
        ...
        def get_offset_geometry(
            ...
            offset_geometry = (
                ...
                .simplify(tolerance)
                .buffer(
                    offset - buffer_distance, cap_style=cap_style, join_style=join_style
                )
                .simplify(tolerance)
            )
            ...
            if (
                (
                    max_polygons
                    and type(offset_geometry) == MultiPolygon
                    and len(offset_geometry.geoms) > max_polygons
                )
                or (
                    max_vertices
                    and type(offset_geometry) == MultiPolygon
                ...
                or (
                    max_vertices
                    and type(offset_geometry) == Polygon
                    and (
        ...
        if len(self.data_variable_list) > 1:
            logger.debug(
                "Multiband dataset. Polygon generation must take into account nulls that appear on each band."
            )
        combined_array = None
        combined_array = np.empty(
            [int(ceil(size / downsampling_stride)) for size in self.data_variable.shape]
        )
        for band in self.data_variable_list:
            logger.debug("Computing padded mask")
            mask_slice = tuple(
                [
                    slice(None, None, downsampling_stride)
                    for _size in self.data_variable.shape
                ]
            )
            padded_mask = np.zeros(
                shape=[
                    int(ceil(size / downsampling_stride)) + 2
                    for size in self.data_variable.shape
                ],
                dtype=bool,
            )
        ...
        if data_proportion >= MAX_DATA_PROPORTION:
            logger.debug(
                "More than {:.2f}% of pixels contain data - assuming full grid coverage".format(
                    data_proportion * 100
                )
            )
        ...
        logger.debug(
            "Generating contours for grid of size {}".format(
                " x ".join(str(size) for size in self.data_variable.shape)
            )
            + (
                " downsampled with stride {}.".format(downsampling_stride)
                if downsampling_stride > 1
                else "."
            )
        )
        contours = [
            (contour - PAD_WIDTH) * downsampling_stride
            # Shift for padding and compensate for downsampling_stride
            for contour in measure.find_contours(
                padded_mask, 0.5, fully_connected="high"
            )
        ]
        # logger.debug('contours = {}'.format(contours))

        concave_hull = MultiPolygon(
            [contour_to_polygon(contour) for contour in contours]
        ).simplify(tolerance)
        concave_hull = get_offset_geometry(
            concave_hull,
            buffer_distance,
            offset,
            tolerance,
        ...
        if self.debug:
            if type(concave_hull) == MultiPolygon:
                ...
                vertex_count = sum(
                    [len(polygon.exterior.coords) for polygon in concave_hull]
                )
                ...
                raise ValueError(
                    "Unexpected type of geometry: {}".format(type(concave_hull))
                )
            logger.debug(
                "Final shape has {} vertices in {} polygons.".format(
    ...
    def get_dimension_ranges(self, bounds, bounds_wkt=None):
        ...
        if isinstance(bounds, BaseGeometry):  # Process shapely (multi)polygon bounds
            bounds_ordinates = (
                bounds.bounds
            )  # Obtain [<xmin>, <ymin>, <xmax>, <ymax>] from Shapely geometry
        else:
            bounds_ordinates = (
                bounds  # Use provided [<xmin>, <ymin>, <xmax>, <ymax>] parameter
            )

        if bounds_wkt is not None:  # Reproject bounds to native CRS if required
            bounds_ordinates = get_reprojected_bounds(
                bounds_ordinates, bounds_wkt, self.wkt
            )
        ...
        try:
            ...
            for dim_index in range(2):
                ...
                subset_indices = np.where(
                    np.logical_and(
                        self.dimension_arrays[1 - dim_index]
                        >= bounds_ordinates[dim_index],
                        self.dimension_arrays[1 - dim_index]
                        <= bounds_ordinates[dim_index + 2],
                    )
                )[0]
                ...
                logger.debug(
                    "self.dimension_arrays[{}].shape = {}".format(
                        1 - dim_index, self.dimension_arrays[1 - dim_index].shape
                    )
                )
                ...
                logger.debug(
                    'dim_range_dict["{}"] = {}'.format(
                        dimension_names[1 - dim_index],
                        dim_range_dict[dimension_names[1 - dim_index]],
                    )
                )
    ...
    def copy(
        ...
        try:
            ...
            if dim_range_dict:  # Subsets were requested
                ...
                logger.debug(
                    "Old first coordinate = ({}, {})".format(
                        self.x_variable[0], self.y_variable[0]
                    )
                )
                logger.debug(
                    "New first coordinate = ({}, {})".format(
                        new_ncgu.x_variable[0], new_ncgu.y_variable[0]
                    )
                )
                geo_transform = new_ncgu.GeoTransform
                logger.debug("Old GeoTransform = {}".format(geo_transform))
                for gt_ordinate_index, ordinate_variable, gt_pixel_width_index in [
                    (0, new_ncgu.x_variable, 1),
                    (3, new_ncgu.y_variable, 5),
                ]:
                    if ordinate_variable.dimensions[0] in dim_range_dict.keys():
                        geo_transform[gt_ordinate_index] = ordinate_variable[
                            0
                        ].item() - (
                            geo_transform[gt_pixel_width_index] / 2.0
                        )  # Subtract half pixel width
            ...
            if invert_y == new_ncgu.y_inverted:
                logger.debug(
                    "{} does not require any alteration to make y-axis inversion {}".format(
                        nc_out_path, invert_y
                    )
                )
                return
            ...
            assert (
                new_ncgu.y_variable
            ), "No Y-axis indexing variable defined in {}".format(nc_out_path)

            assert (
                len(new_ncgu.y_variable.shape) == 1
            ), "Y-axis indexing variable should be 1D"
            ...
            for variable_name, variable in new_dataset.variables.items():
                if y_axis_dimension_name in variable.dimensions:
                    logger.debug(
                        "Inverting Y dimension of variable {}".format(variable_name)
                    )
                    variable_slices = [
                        (
                            slice(None, None, -1)
                            if dimension_name == y_axis_dimension_name
    ...
    def set_global_attributes(
        ...
        try:
            ...
            attribute_dict["pixel_count"] = np.array(self.pixel_count).astype(
                "int32"
            )  # same as dimensions

            gda_wkt = get_spatial_ref_from_wkt(
                METADATA_CRS
            ).ExportToPrettyWkt()  # this is wkt of (currently) gda94
            attribute_dict["geospatial_bounds_crs"] = gda_wkt
            metadata_bbox = get_reprojected_bounds(
                self.bounds, self.wkt, gda_wkt
            )  # Reproject bounding box from native CRS to metadata CRS
            ...
            attribute_dict["nominal_pixel_size_lon_degrees"] = (
                self.nominal_pixel_degrees[0]
            )  # lon
            attribute_dict["nominal_pixel_size_lat_degrees"] = (
                self.nominal_pixel_degrees[1]
            )  # lat
            attribute_dict["nominal_pixel_size_x_metres"] = self.nominal_pixel_metres[
                0
            ]  # x
            attribute_dict["nominal_pixel_size_y_metres"] = self.nominal_pixel_metres[
                1
            ]  # y
            attribute_dict["geospatial_lon_resolution"] = self.nominal_pixel_degrees[
                0
            ]  # lon
            attribute_dict["geospatial_lat_resolution"] = self.nominal_pixel_degrees[
                1
            ]  # lat
            ...
            if compute_shape:
                ...
                if clockwise_polygon_orient:
                    logger.info(
                        "Keeping default 'geospatial_bounds' as a clockwise orientation"
                    )
                    wkt_polygon = shapely.geometry.polygon.orient(Polygon(pol), -1.0)
                else:  # reverse polygon coordinates - anti-clockwise
                    logger.info(
                        "Setting 'geospatial_bounds' to anticlockwise orientation"
                    )
                    wkt_polygon = shapely.geometry.polygon.orient(Polygon(pol), 1.0)
            ...
            logger.error(
                "Unable to set geometric metadata attributes in netCDF grid dataset"
            )
            raise

    def set_variable_actual_range_attribute(
        self,
        iterate_through_data=False,
        ...
        for variable_name, variable in self.netcdf_dataset.variables.items():
            ...
            logger.debug(
                "Total num of pixels: {}".format(
                    self.pixel_count[0] * self.pixel_count[1]
                )
            )
            logger.debug(
                "num_pixels_to_trigger_iterating: {}".format(
                    num_pixels_to_trigger_iterating
                )
            )
            if (
                iterate_through_data
                and self.pixel_count[0] * self.pixel_count[1]
                > num_pixels_to_trigger_iterating
            ):
                logger.debug(
                    "Pixel count is greater than array_total_size setting. Iterating through variable to find min, max"
                )
                min, max = self.iterate_through_data_chunks_and_find_mins_and_maxs(
                    variable=variable, num_of_chunks=num_of_chunks
                )
                ...
                logger.debug(
                    "{}.actual_range = {}".format(variable_name, variable.actual_range)
                )
            logger.debug("actual range: {}: ".format(variable.actual_range))

    def iterate_through_data_chunks_and_find_mins_and_maxs(
        self, variable, num_of_chunks=10
    ):
        ...
        for i in range(num_of_chunks):
            ...
            if data_chunk.mask.all():
                logger.debug(
                    "data chunk contains only masked data. Continuing to next chunk"
                )
                continue
...
def _get_query_params(index_array, start_index, data_variable, max_bytes):
    ...
    nbytes = dbytes
    bbox = [
        index_array[start_index, 0],
        index_array[start_index, 0],
        index_array[start_index, 1],
        index_array[start_index, 1],
    ]
    while end_index < len(index_array) and nbytes <= max_bytes:
        ...
        if y < bbox[2]:
            ...
            new_bbox[3] = y
        nbytes = (
            dbytes * (new_bbox[1] - new_bbox[0] + 1) * (new_bbox[3] - new_bbox[2] + 1)
        )
        if nbytes > max_bytes:
            break
        bbox = new_bbox
...
def main():
    ...
    parser.add_argument(
        "--chunkspec",
        help="comma-separated list of <dimension_name>/<chunk_size> specifications",
        type=str,
    )
    parser.add_argument(
        "--complevel",
        help="Compression level for chunked variables as an integer 0-9. Default is 4",
        type=int,
        default=4,
    )
    parser.add_argument(
        "-i",
        "--invert_y",
        help="Store copy with y-axis indexing Southward positive (grids only)",
        type=str,
    )
    parser.add_argument(
        "-d",
    ...
    if args.chunkspec:
        chunk_spec = {
            dim_name: int(chunk_size)
            for dim_name, chunk_size in [
                chunk_spec_string.strip().split("/")
                for chunk_spec_string in args.chunkspec.split(",")
            ]
        }
    else:
        chunk_spec = None

    ncgu = NetCDFGridUtils(args.input_path, debug=args.debug)
        ...
        variable_options_dict=(
            {
                variable_name: {
                    "chunksizes": [
                        chunk_spec.get(dimension) for dimension in variable.dimensions
                    ],
