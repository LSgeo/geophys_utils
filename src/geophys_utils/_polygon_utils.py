...
def get_grid_edge_points(grid_array, dimension_ordinates, nodata_value, max_bytes=None):
    ...
    for piece_array, array_offset in array_pieces(grid_array, max_bytes=max_bytes):
        dimension_subset = [
            dimension_ordinates[dim_index][
                array_offset[dim_index] : array_offset[dim_index]
        ...
        piece_slices = tuple(
            [
                slice(
                    *[
                        (
                            0 if array_offset[dim_index] == 0 else 1
                        ),  # Trim padding away if not on lower limit
        ...
        piece_array = piece_array[
            piece_slices
        ]  # Trim away padding on internal edges if required
        # logger.debug('piece_array = {}'.format(piece_array))
        ...
        edge_mask = ndimage.filters.maximum_filter(
            piece_array, size=2, mode="constant", cval=False
        ...
        edge_point_indices = edge_point_indices[
            np.where(np.all((edge_point_indices < unpadded_piece_shape), axis=1))
        ]

        # logger.debug('edge_point_indices.shape = {}\nedge_point_indices = {}'.format(edge_point_indices.shape, edge_point_indices))

        if edge_point_indices.shape[0]:  # If any edge points found
            piece_edge_points = np.zeros(
                edge_point_indices.shape, dimension_ordinates[0].dtype
            )
            ...
            for dim_index in range(2):
                piece_edge_points[:, 1 - dim_index] = dimension_subset[dim_index][
                    edge_point_indices[:, dim_index]
                ]
                pixel_offset[1 - dim_index] = (
                    dimension_subset[dim_index][1] - dimension_subset[dim_index][0]
                ) / 2.0

            # logger.debug('pixel_offset = {}'.format(pixel_offset))
            # logger.debug('piece_edge_points = {}'.format(piece_edge_points))
            piece_edge_points = piece_edge_points - np.array(
                pixel_offset
...
def get_netcdf_edge_points(netcdf_dataset, max_bytes=None):
    ...
    dimension_ordinates = [
        netcdf_dataset.variables[data_variable.dimensions[dim_index]]
        for dim_index in range(2)
    ]
    nodata_value = data_variable._FillValue

    return get_grid_edge_points(
        data_variable, dimension_ordinates, nodata_value, max_bytes
    )
...
def netcdf2convex_hull(netcdf_dataset, max_bytes=None):
    ...
    try:
        grid_mapping_variable = [
            variable
            for variable in netcdf_dataset.variables.values()
            if hasattr(variable, "GeoTransform")
        ][0]
    except:
        raise Exception(
    ...
    GeoTransform = [
        float(number)
        for number in grid_mapping_variable.GeoTransform.strip().split(" ")
    ]
    avg_pixel_size = (abs(GeoTransform[1]) + abs(GeoTransform[5])) / 2.0

    return points2convex_hull(
        get_netcdf_edge_points(netcdf_dataset, max_bytes),
        avg_pixel_size,
        avg_pixel_size,
    )
