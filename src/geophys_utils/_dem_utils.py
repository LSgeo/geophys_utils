...
class DEMUtils(NetCDFGridUtils):
    ...
    def get_pixel_size(self, index_tuple):
        ...
        coord_transform_to_latlong = osr.CoordinateTransformation(
            native_spatial_reference, latlong_spatial_reference
        )
        ...
        logger.debug(
            "For X size: (lon1, lat1) = (%f, %f), (lon2, lat2) = (%f, %f)",
            lon1,
            lat1,
            lon2,
            lat2,
        ...
        logger.debug(
            "For Y size: (lon1, lat1) = (%f, %f), (lon2, lat2) = (%f, %f)",
            lon1,
            lat1,
            lon2,
            lat2,
    ...
    def get_pixel_size_grid(self, source_array, offsets):
        ...
        pixel_size_grid = numpy.zeros(
            shape=(source_array.shape[0], source_array.shape[1], 2)
        ).astype(source_array.dtype)
    ...
    def create_dzdxy_arrays(self, elevation_array, offsets):
        ...
        dzdx_array = sobel(elevation_array, axis=1) / (8.0 * native_pixel_x_size)
        dzdy_array = sobel(elevation_array, axis=0) / (8.0 * native_pixel_y_size)

        if pixels_in_m():
            print(
                "Pixels are a uniform size of {} x {} metres.".format(
                    native_pixel_x_size, native_pixel_y_size
                )
            )
            ...
            print(
                "Pixels are of varying sizes. Computing and applying pixel size arrays."
            )
            ...
            dzdx_array = numexpr.evaluate(
                "dzdx_array * native_pixel_x_size / pixel_x_metres"
            )
            dzdy_array = numexpr.evaluate(
                "dzdy_array * native_pixel_y_size / pixel_y_metres"
            )
    ...
    def create_slope_and_aspect(self, slope_path=None, aspect_path=None, overlap=4):
        ...
        slope_variable.long_name = (
            "slope expressed in degrees from horizontal (0=horizontal, 90=vertical)"
        )
        ...
        for piece_array, offsets in array_pieces(
            self.data_variable,
            ...
            print(
                "Processing array of shape {} at {}".format(piece_array.shape, offsets)
            )

            if type(piece_array) == numpy.ma.masked_array:
                piece_array = (
                    piece_array.data
                )  # Convert from masked array to plain array
            ...
            source_slices = [
                slice(0, piece_array.shape[dim_index]) for dim_index in range(2)
            ]
            ...
            source_slices = [
                slice(
                    (
                        0
                        if dest_slices[dim_index].start < overlap
                        else source_slices[dim_index].start + overlap
                    ),
                    (
                        piece_array.shape[dim_index]
                        if (
                            self.data_variable.shape[dim_index]
                            - dest_slices[dim_index].stop
                        )
                        < overlap
                        else source_slices[dim_index].stop - overlap
                    ),
                )
                for dim_index in range(2)
            ]
            ...
            dest_slices = [
                slice(
                    (
                        0
                        if dest_slices[dim_index].start < overlap
                        else dest_slices[dim_index].start + overlap
                    ),
                    (
                        self.data_variable.shape[dim_index]
                        if (
                        ...
                        < overlap
                        else dest_slices[dim_index].stop - overlap
                    ),
                )
                for dim_index in range(2)
            ]
            ...
            print(
                "Writing slope array of shape %s at %s".format(
                    tuple(
                        [
                            dest_slices[dim_index].stop - dest_slices[dim_index].start
                            for dim_index in range(2)
                        ]
                    ),
                    tuple([dest_slices[dim_index].start for dim_index in range(2)]),
                )
            )
            ...
            print(
                "Writing aspect array of shape {} at {}".format(
                    tuple(
                        [
                            dest_slices[dim_index].stop - dest_slices[dim_index].start
                            for dim_index in range(2)
