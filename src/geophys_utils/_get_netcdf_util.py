        ...
        try:
            try:
                ...
                if not _netcdf_dataset.startswith("http"):
                    raise
                _netcdf_dataset = netCDF4.Dataset(
                    netcdf_dataset + "#fillmismatch", open_mode
                )
    ...
    # Dataset has 2D (or higher dimensionality) variable with grid_mapping attribute and indexing variables
        ...
        len(
            [
                ...
                )  # "crs" or similar variable specified
                and variable.grid_mapping
                in netcdf_dataset.variables.keys()  # "crs" or similar variable exists
                and len(variable.dimensions) >= 2  # Data variable is a grid
                and set(variable.dimensions)
                <= set(netcdf_dataset.variables.keys())  # Indexing variables exist
            ]
        )
        > 0
    ):
