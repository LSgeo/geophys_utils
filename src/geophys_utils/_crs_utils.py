...
def get_spatial_ref_from_wkt(wkt_or_crs_name):
    ...
    if not result:
        logger.debug(
            "CRS determined using SpatialReference.SetFromUserInput({})".format(
                wkt_or_crs_name
            )
        )
        return spatial_ref
    ...
    if not result:
        logger.debug(
            "CRS determined using SpatialReference.SetWellKnownGeogCS({})".format(
                modified_crs_name
            )
        )
        return spatial_ref
    ...
    if match:
        ...
        if not result:
            logger.debug(
                "CRS determined using SpatialReference.ImportFromEPSG({})".format(
                    epsg_code
                )
            )
            return spatial_ref
    ...
    if not result:
        ...
        logger.debug(
            "UTM CRS determined using SpatialReference.SetWellKnownGeogCS({}) (zone{})".format(
                modified_crs_name, utm_zone
            )
        )
        return spatial_ref
...
def get_coordinate_transformation(from_wkt, to_wkt):
    ...
    if (
        not from_spatial_ref
        or not to_spatial_ref
        or from_spatial_ref.ExportToWkt() == to_spatial_ref.ExportToWkt()
    ):
        return None
...
def get_utm_wkt(coordinate, from_wkt):
    ...
    latlon_coord = (
        coordinate
        if latlon_coord_trans is None
        else latlon_coord_trans.TransformPoints(coordinate_array)[0][0:2]
    )

    # Set UTM coordinate reference system
    utm_spatial_ref = SpatialReference()
    utm_spatial_ref.SetWellKnownGeogCS("WGS84")
    utm_spatial_ref.SetUTM(
        utm_getZone(latlon_coord[0]), utm_isNorthern(latlon_coord[1])
    )
...
def transform_coords(coordinates, from_wkt, to_wkt):
    ...
    POINT_CHUNK_SIZE = (
        1048576  # Number of points to check at any one time to keep memory usage down
    )

    coord_trans = get_coordinate_transformation(
        from_wkt, to_wkt
    )  # Transform from specified CRS to native CRS
    ...
    while chunk_start_index < len(coordinate_array):
        ...
        logger.debug(
            "Transforming coordinates {} to {} of {}".format(
                chunk_start_index, chunk_end_index - 1, len(coordinates)
            )
        )
        transformed_coord_chunk_list.append(
            np.array(
                coord_trans.TransformPoints(
                    coordinate_array[slice(chunk_start_index, chunk_end_index)]
                )
            )[:, 0:2]
        )
