...
def line_length(line):
    ...
    """
    return math.sqrt(
        math.pow(line[1][0] - line[0][0], 2.0) + math.pow(line[1][1] - line[0][1], 2.0)
    )
...
def point_along_line(line, distance):
    ...
    return tuple(
        [
            line[0][dim_index] + proportion * (line[1][dim_index] - line[0][dim_index])
...
def utm_coords(coordinate_array, wkt):
    ...
    native_centre_coords = (
        np.nanmean(coordinate_array[:, 0]),
        np.nanmean(coordinate_array[:, 1]),
    )
...
def coords2distance(coordinate_array):
    ...
    for coord_index in range(1, coord_count):
        ...
        distance = math.sqrt(
            math.pow(point[0] - last_point[0], 2.0)
            + math.pow(point[1] - last_point[1], 2.0)
        )
...
def sample_transect(transect_vertices, wkt, sample_metres):
    ...
    for vertex_index in range(len(utm_transect_vertices) - 1):
        utm_line = (
            utm_transect_vertices[vertex_index],
            utm_transect_vertices[vertex_index + 1],
        )
        ...
        try:
            sample_point_array = np.stack(
                [
                    np.linspace(
                        start_point[dim_index], end_point[dim_index], sample_count + 1
                    )
                    for dim_index in range(2)
                ]
            ).transpose()
    ...
    return transform_coords(sample_points, nominal_utm_wkt, wkt), sample_metres
