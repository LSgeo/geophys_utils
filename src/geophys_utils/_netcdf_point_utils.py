...
from geophys_utils._crs_utils import (
    transform_coords,
    get_utm_wkt,
    get_reprojected_bounds,
    get_spatial_ref_from_wkt,
)
...
try:
    ...
    logger.debug(
        "Unable to import memcache. AWS-specific functionality will not be enabled"
    )
    memcache = None
...
SHAPE_BUFFER_DISTANCE = (
    0.02  # Distance to buffer (kerf) shape out then in again (in degrees)
)
...
class NetCDFPointUtils(NetCDFUtils):
    ...
    def __init__(
        ...
        super().__init__(netcdf_dataset=netcdf_dataset, debug=debug)
        ...
        self.cache_path = (
            cache_path
            or os.path.join(
                os.path.join(tempfile.gettempdir(), "NetCDFPointUtils"),
                re.sub("\W", "_", os.path.splitext(self.nc_path)[0]),
            )
            + "_cache.nc"
        )

        self.cache_basename = os.path.join(
            self.cache_path, re.sub("\W", "_", os.path.splitext(self.nc_path)[0])
        )
    ...
    def fetch_array(self, source_variable, dest_array=None):
        ...
        pieces_required = int(
            math.ceil((source_variable[0].itemsize * source_len) / self.max_bytes)
        )
        ...
        if pieces_required > 1 and hasattr(source_variable, "_ChunkSizes"):
            chunk_size = (
                source_variable._ChunkSizes
                if type(source_variable._ChunkSizes) in [int, np.int32]
                else source_variable._ChunkSizes[0]
            )
            chunk_count = max(max_elements // chunk_size, 1)
            max_elements = min(chunk_count * chunk_size, max_elements)
            pieces_required = int(math.ceil(source_len / max_elements))

        logger.debug(
            "Fetching {} pieces containing up to {} {} array elements.".format(
                pieces_required, max_elements, source_variable.name
            )
        )
        ...
        while start_index < source_len:
            ...
            logger.debug(
                "Retrieving {} array elements {}:{}".format(
                    source_variable.name, start_index, end_index
                )
            )
    ...
    def get_polygon(self):
        ...
        return (
            ...
            + ", ".join(
                [
                    " ".join(["%.4f" % ordinate for ordinate in coordinates])
    ...
    def get_spatial_mask(self, bounds, bounds_wkt=None):
        ...
        def get_intersection_mask(points, geometry):
            ...
            while chunk_start_index < len(points):
                ...
                logger.debug(
                    "Checking spatial containment for points {} to {} of {}".format(
                        chunk_start_index, chunk_end_index - 1, len(points)
                    )
                )

                mp_intersect = MultiPoint(
                    points[slice(chunk_start_index, chunk_end_index)]
                ...
                if isinstance(mp_intersect, MultiPoint):
                    intersection_points = np.array(
                        [point.coords for point in mp_intersect.geoms]
                    )
                    logger.debug(
                        "Object type (MultiPoint) returned by MultiPoint intersection method"
                    )
                elif isinstance(mp_intersect, Point):
                    intersection_points = np.array([mp_intersect.coords])
                    logger.debug(
                        "Object type (Point) returned by MultiPoint intersection method"
                    )
                else:
                    raise BaseException(
                        "Object type returned by MultiPoint intersection method not supported"
                    )
                ...
                logger.debug(
                    "Computing partial mask from {} intersection points".format(
                        len(intersection_points)
                    )
                )
                _x_values, x_indices, _x_intersection_indices = np.intersect1d(
                    points.flatten()[0::2],
                    intersection_points.flatten()[0::2],
                    return_indices=True,
                )
                _y_values, y_indices, _y_intersection_indices = np.intersect1d(
                    points.flatten()[1::2],
                    intersection_points.flatten()[1::2],
                    return_indices=True,
                )
                intersection_indices = np.intersect1d(
                    x_indices, y_indices, return_indices=False
                )
        ...
        coordinates = (
            self.xycoords
        )  # Don't transform these - do all spatial operations in native CRS
        ...
        if isinstance(bounds, BaseGeometry):  # Process shapely (multi)polygon bounds
            if bounds_wkt is None:
                ...
                native_crs_bounds = transform(
                    (lambda x, y: transform_coords([x, y], bounds_wkt, self.wkt)),
                    bounds,
                )
            ...
            bounds_half_size = (
                abs(
                    np.array(
                        [
                            native_crs_bounds.bounds[2] - native_crs_bounds.bounds[0],
                            native_crs_bounds.bounds[3] - native_crs_bounds.bounds[1],
                        ]
                    )
                )
                / 2.0
            ...
            mask = np.all(
                ne.evaluate("abs(coordinates - bounds_centroid) <= bounds_half_size"),
                axis=1,
            )
            logger.debug(
                "{}/{} points found in initial bounding box intersection".format(
                    np.count_nonzero(mask), len(coordinates)
                )
            )

            # Apply sub-mask for all points within bounds geometry
            (mask[mask])[
                ~get_intersection_mask(coordinates[mask], native_crs_bounds)
            ] = False
            ...
            assert (
                len(bounds) == 4
            ), "Invalid bounds iterable: {}. Must be of form [<xmin>, <ymin>, <xmax>, <ymax>]".format(
                bounds
            )
            ...
            native_crs_bounds = np.array(
                transform_coords(
                    np.array(bounds).reshape((2, 2)), bounds_wkt, self.wkt
                ).reshape((4,))
            )

            if (
                self.bounds[0] >= native_crs_bounds[0]
            ...
            bounds_half_size = (
                abs(
                    np.array(
                        [
                            native_crs_bounds[2] - native_crs_bounds[0],
                            native_crs_bounds[3] - native_crs_bounds[1],
                        ]
                    )
                )
                / 2.0
            ...
            bounds_centroid = (
                np.array([native_crs_bounds[0], native_crs_bounds[1]])
                + bounds_half_size
            )
        ...
        logger.debug(
            "{}/{} points found in final mask".format(
                np.count_nonzero(mask), len(coordinates)
            )
        )
        return mask
    ...
    def grid_points(
        ...
        if native_grid_bounds:
            reprojected_grid_bounds = get_reprojected_bounds(
                native_grid_bounds, self.wkt, grid_wkt
            )
        elif reprojected_grid_bounds:
            native_grid_bounds = get_reprojected_bounds(
        ...
        spatial_subset_mask = self.get_spatial_mask(
            get_reprojected_bounds(expanded_grid_bounds, grid_wkt, self.wkt)
        )
        ...
        grids = {}
        for variable in [
            self.netcdf_dataset.variables[var_name] for var_name in variables
        ]:
        ...
        geotransform = [
            pixel_centre_bounds[0] - grid_resolution / 2.0,
            grid_resolution,
            0,
    ...
    def utm_grid_points(
        ...
        native_centre_coords = [
            (native_grid_bounds[dim_index] + native_grid_bounds[dim_index + 2]) / 2.0
            for dim_index in range(2)
        ]
        utm_wkt = get_utm_wkt(native_centre_coords, self.wkt)

        return self.grid_points(
            grid_resolution=utm_grid_resolution,
            variables=variables,
    ...
    def get_concave_hull(
        self, to_wkt=None, smoothness=None, clockwise_polygon_orient=False
    ):
        """\
    ...
    def nearest_neighbours(
        ...
        if max_distance:  # max_distance has been specified
            ...
            point_indices = np.where(np.logical_and(spatial_mask, secondary_mask))[0]

            if not len(point_indices):
                logger.debug(
                    "No points within distance {} of {}".format(
    ...
    def get_lookup_mask(
        ...
        if lookup_variable_name:
            ...
            if lookup_variable.shape == () or (
                (len(lookup_variable.shape) == 1) and (lookup_variable.dtype == "|S1")
            ):  # Scalar or string array
                dimension = self.netcdf_dataset.get(indexing_dimension)
                assert dimension, "Invalid indexing_dimension {} specified".format(
                    indexing_dimension
                )
                # Repeat boolean value across dimension size
                return np.array(
                    [lookup_variable[:] in lookup_value_list] * dimension.size
                )

            indexing_variable_name = (
                indexing_variable_name or lookup_variable_name + "_index"
            )

            try:
                indexing_variable = self.netcdf_dataset.variables[
                    indexing_variable_name
                ]
            except:
                raise BaseException(
                    "indexing_variable_name not supplied and cannot be inferred"
                )
            ...
            if hasattr(indexing_variable, "lookup"):
                ...
                raise BaseException(
                    "lookup_variable_name not supplied and cannot be inferred"
                )

            lookup_variable = self.netcdf_dataset.variables[lookup_variable_name]
        else:
            raise BaseException(
                "Must supply either lookup_variable_name or indexing_variable_name"
            )

        # Handle special case for string arrays via OPeNDAP
        if (
            self.opendap
            and (lookup_variable.dtype == "S1")
            and (len(lookup_variable.shape) == 2)
        ):
            # Convert 2D byte array into 1D array of unicode strings - needed for OPeNDAP
            lookup_array = np.array(
                [
                    bytestring[bytestring != b""].tostring().decode("UTF8")
                    for bytestring in lookup_variable[:]
                ]
            )
            # OPeNDAP will truncate strings to 64 characters - truncate search strings to match
            lookup_indices = np.arange(lookup_array.shape[0])[
                np.in1d(
                    lookup_array,
                    np.array(
                        [lookup_value[0:64] for lookup_value in lookup_value_list]
                    ),
                )
            ]
        else:
    ...
    def get_index_mask(
        ...
        start_index_variable_name = (
            start_index_variable_name or lookup_variable_name + "_start_index"
        )
        try:
            start_index_variable = self.netcdf_dataset.variables[
                start_index_variable_name
            ]
        except:
            raise BaseException(
                "start_index_variable_name not supplied and cannot be inferred"
            )
        ...
        try:
            ...
            raise BaseException(
                "count_variable_name not supplied and cannot be inferred"
            )

        point_count = point_count or self.netcdf_dataset.dimensions["point"].size

        lookup_indices = np.arange(lookup_variable.shape[0])[
            np.in1d(lookup_variable[:], lookup_value_list)
        ]
        logger.debug("lookup_indices: {}".format(lookup_indices))
        ...
        for lookup_index in range(len(lookup_indices)):
            index_mask[
                start_indices[lookup_index] : start_indices[lookup_index]
                + counts[lookup_index]
            ] = True

        return index_mask

    def expand_lookup_variable(
        self,
        lookup_variable_name="line",
        indexing_variable_name=None,
        start_index=0,
        ...
        end_index = (
            end_index or indexing_variable.shape[0]
        )  # Usually this will be the point count
        ...
        result_array = lookup_variable[:][
            indexing_variable[start_index:end_index][subset_mask]
        ]  # Need to index numpy array, not netCDF variable
        ...
        if result_array.dtype == "S1":
            result_array = np.array(
                [
                    bytestring[bytestring != b""].tostring().decode("UTF8")
                    for bytestring in result_array
                ]
            )

        return result_array

    def chunk_point_data_generator(
        self,
        start_index=0,
        end_index=0,
        field_list=None,
        ...
        if not index_range:
            logger.debug(
                "No points to retrieve for point indices {}-{}: All masked out".format(
        ...
        if not field_list:
            field_list = [
                ...
                if (
                    not len(variable.dimensions)  # Scalar variable
                    or variable.dimensions[0]
                    == "point"  # Variable is of point dimension
                    or (
                        variable.dimensions[0] + "_index"
                        in self.netcdf_dataset.variables.keys()  # Variable has an index variable
                        and len(
                            self.netcdf_dataset.variables[
                                variable.dimensions[0] + "_index"
                            ].dimensions
                        )  # index variable is not a scalar
                        and self.netcdf_dataset.variables[
                            variable.dimensions[0] + "_index"
                        ].dimensions[0]
                        == "point"  # index variable is of point dimension
                    )
                )
        ...
        for variable_name in field_list:
            ...
            if variable is None:
                logger.warning(
                    "Variable {} does not exist. Skipping.".format(variable_name)
                )
                continue
            ...
            if len(variable.shape) == 0:
                # Skip CRS variable
                if variable_name in NetCDFUtils.CRS_VARIABLE_NAMES or re.match(
                    "ga_.+_metadata", variable_name
                ):
                    continue
                ...
                if (
                    ...
                    memory_cache[variable_name] = self.expand_lookup_variable(
        ...
        logger.debug(
            "{} points read for point indices {}-{}".format(
                index_range, start_index, end_index - 1
            )
        )

    def all_point_data_generator(
        self,
        field_list=None,
        mask=None,
        ...
        for chunk_index in range(self.point_count // read_chunk_size + 1):
            for line in self.chunk_point_data_generator(
                field_list=field_list,
                start_index=chunk_index * read_chunk_size,
                end_index=min((chunk_index + 1) * read_chunk_size, self.point_count),
                mask=mask,
                ...
                yield_variable_attributes_first = (
                    False  # Only yield variable attributes from the first chunk
                )
        ...
        logger.debug(
            "{} points read from netCDF file {}".format(point_count, self.nc_path)
        )
    ...
    def get_xy_coord_values(self):
        ...
        xycoord_values = np.zeros(
            shape=(len(self.x_variable), 2), dtype=self.x_variable.dtype
        )
        ...
        if hasattr(self.x_variable, "_FillValue"):
            xycoord_values[:, 0][
                xycoord_values[:, 0] == self.x_variable._FillValue
            ] = np.nan
        if hasattr(self.y_variable, "_FillValue"):
            xycoord_values[:, 1][
                xycoord_values[:, 1] == self.y_variable._FillValue
            ] = np.nan
    ...
    def xycoords(self):
        ...
        if self.enable_memory_cache and self._xycoords is not None:
            ...
            if xycoords is not None:
                ...
                logger.debug(
                    "key not found at {}. adding key and value".format(coord_cache_key)
                )
            ...
            if os.path.isfile(self.cache_path):
                ...
                if "xycoords" in cache_dataset.variables.keys():
                    ...
                    logger.debug(
                        "Read {} coordinates from cache file {}".format(
                            xycoords.shape[0], self.cache_path
                        )
                    )
                else:
                    logger.debug(
                        "Unable to read xycoords variable from netCDF cache file {}".format(
                            self.cache_path
                        )
                    )
                cache_dataset.close()
                ...
                logger.debug(
                    "NetCDF cache file {} does not exist".format(self.cache_path)
                )
            ...
            if xycoords is None:
                ...
                if "point" not in cache_dataset.dimensions.keys():
                    cache_dataset.createDimension(
                        dimname="point", size=xycoords.shape[0]
                    )
                ...
                if "xycoords" not in cache_dataset.variables.keys():
                    cache_dataset.createVariable(
                        "xycoords",
                        xycoords.dtype,
                        dimensions=["point", "xy"],
                        **self.CACHE_VARIABLE_PARAMETERS,
                    )
                cache_dataset.variables["xycoords"][
                    :
                ] = xycoords  # Write coords to cache file
                cache_dataset.close()
                logger.debug(
                    "Saved {} coordinates to cache file {}".format(
                        xycoords.shape[0], self.cache_path
                    )
                )
    ...
    def point_variables(self):
        ...
        if not self._point_variables:
            ...
            self._point_variables = list(
                [
                    var_name
                    for var_name in self.netcdf_dataset.variables.keys()
    ...
    def data_variable_list(self):
        ...
        if not self._data_variable_list:
            ...
            self._data_variable_list = [
                key
                for key, value in self.netcdf_dataset.variables.items()
    ...
    def kdtree(self):
        ...
        if not self._kdtree:
            logger.debug(
                "Indexing full dataset with {} points into KDTree...".format(
                    self.xycoords.shape[0]
                )
            )
    ...
    def copy(
        ...
        try:
            ...
            logger.debug(
                "Reprojecting {} coordinates in new dataset".format(
                    len(new_ncpu.x_variable)
                )
            )
            ...
            if crs_variable_name != new_ncpu.crs_variable.name:
                logger.debug(
                    "Renaming {} variable to {}".format(
                        new_ncpu.crs_variable.name, crs_variable_name
                    )
                )
                new_dataset.renameVariable(
                    new_ncpu.crs_variable.name, crs_variable_name
                )
                ...
                if crs_variable_name == "crs":  # Geodetic
                    ...
                    raise BaseException(
                        'Unhandled crs variable name "{}"'.format(crs_variable_name)
                    )

                logger.debug(
                    "Renaming {} & {} variables to {} & {}".format(
                        new_ncpu.x_variable.name, new_ncpu.y_variable.name, *xy_varnames
                    )
                )
    ...
    def set_global_attributes(
        self, compute_shape=False, clockwise_polygon_orient=False
    ):
        """\
        ...
        try:
            ...
            assert (
                metadata_srs.IsGeographic()
            ), "Unable to set geodetic parameters for this dataset"
            ...
            logger.debug(
                "Setting global geometric metadata attributes in netCDF point dataset with {} points".format(
            ...
            if compute_shape:
                try:
                    ...
                    try:
                        self.netcdf_dataset.geospatial_bounds = shapely.wkt.dumps(
                            Polygon(
                                [
                                    [
                                        attribute_dict["geospatial_lon_min"],
                                        attribute_dict["geospatial_lat_min"],
                                    ],
                                    [
                                        attribute_dict["geospatial_lon_max"],
                                        attribute_dict["geospatial_lat_min"],
                                    ],
                                    [
                                        attribute_dict["geospatial_lon_max"],
                                        attribute_dict["geospatial_lat_max"],
                                    ],
                                    [
                                        attribute_dict["geospatial_lon_min"],
                                        attribute_dict["geospatial_lat_max"],
            ...
            logger.debug(
                "Writing global attributes to netCDF file".format(
                    self.netcdf_dataset.filepath()
                )
            )
            ...
            logger.debug(
                "Finished setting global geometric metadata attributes in netCDF point dataset"
            )
        except:
            logger.error(
                "Unable to set geometric metadata attributes in netCDF point dataset"
            )
            raise
            ...
            for variable_name, variable in self.netcdf_dataset.variables.items():
                ...
                try:
                    variable.actual_range = np.array(
                        [np.nanmin(variable[:]), np.nanmax(variable[:])],
                        dtype=variable.dtype,
                    )
                    logger.debug(
                        "{}.actual_range = {}".format(
                            variable_name, variable.actual_range
                        )
                    )
                except:
                    logger.warning(
                        "Unable to compute actual_range value for point variable {}".format(
                            variable_name
                        )
                    )
        except:
            logger.error(
                "Unable to set variable actual_range metadata attributes in netCDF point dataset"
            )
            raise
...
def main(debug=True):
    ...
    field_names = [
        variable_attributes[variable_name].get("long_name") or variable_name
