...
SIG_FIGS = OrderedDict(
    [
        ...
        (
            "uint32",
            19,
        ),  # 2147483648 - should be 9, but made 10 because int64 is unsupported
        (
            "uint64",
            30,
        ),  # 9223372036854775808 - Not supported in netCDF3 or netCDF4-Classic
        ("int8", 2),  # 128
        ("int16", 4),  # 32768
        (
            "int32",
...
def decode_aseg_gdf_format(aseg_gdf_format):
    ...
    logger.debug(
        "aseg_gdf_format: {}, columns: {}, aseg_dtype_code: {}, width_specifier: {}, decimal_places: {}".format(
            aseg_gdf_format, columns, aseg_dtype_code, width_specifier, decimal_places
        )
    )
...
def aseg_gdf_format2dtype(aseg_gdf_format):
    ...
    @return decimal_places: Number of fractional digits read from format string
    """
    columns, aseg_dtype_code, width_specifier, decimal_places = decode_aseg_gdf_format(
        aseg_gdf_format
    )
    ...
    # TODO: Remove 'A' after string field handling has been properly implemented
        ...
        for test_dtype, sig_figs in SIG_FIGS.items():
            if (
                test_dtype.startswith("float") and sig_figs >= width_specifier - 2
            ):  # Allow for sign and decimal place
                dtype = test_dtype
                break
        assert dtype, "Invalid floating point format of {}.{}".format(
            width_specifier, decimal_places
        )
    ...
    logger.debug(
        "aseg_dtype_code: {}, columns: {}, width_specifier: {}, decimal_places: {}".format(
...
def variable2aseg_gdf_format(array_variable, decimal_places=None):
    ...
    if not len(array_variable.shape):  # Scalar
        ...
        if dtype == "str":
            ...
            sig_figs = (
                SIG_FIGS[dtype] + 1
            )  # Look up approximate significant figures and add 1
    ...
    if aseg_dtype_code == "I":  # Integer
        ...
        python_format = (
            "{" + ":>{:d}.{:d}f".format(width_specifier, decimal_places) + "}"
        )
        ...
        if decimal_places is not None:
            ...
            logger.debug(
                "decimal_places set to {} from decimal_places {}".format(
                    decimal_places, decimal_places
                )
            )
        elif hasattr(array_variable, "aseg_gdf_format"):
            _columns, _aseg_dtype_code, _integer_digits, decimal_places = (
                decode_aseg_gdf_format(array_variable.aseg_gdf_format)
            )
            decimal_places = min(decimal_places, abs(sig_figs - integer_digits))
            logger.debug(
                "decimal_places set to {} from variable attribute aseg_gdf_format {}".format(
                    decimal_places, array_variable.aseg_gdf_format
                )
            )
        else:  # No aseg_gdf_format variable attribute
            decimal_places = abs(
                sig_figs - integer_digits
            )  # Allow for full precision of datatype
            logger.debug(
                "decimal_places set to {} from sig_figs {} and integer_digits {}".format(
                    decimal_places, sig_figs, integer_digits
                )
            )

        width_specifier = min(
            sign_width + integer_digits + decimal_places + 2, sign_width + sig_figs + 2
        )

        aseg_gdf_format = "{}{}.{}".format(
            aseg_dtype_code, width_specifier, decimal_places
        )
        if aseg_dtype_code == "F":  # Floating point notation
            python_format = (
                "{" + ":>{:d}.{:d}f".format(width_specifier, decimal_places) + "}"
            )  # Add 1 to width for decimal point
        else:  # Exponential notation for 'D' or 'E'
            python_format = (
                "{" + ":>{:d}.{:d}E".format(width_specifier, decimal_places) + "}"
            )  # Add 1 to width for decimal point

    elif aseg_dtype_code == "A":  # String
        if hasattr(array_variable, "aseg_gdf_format"):
            _columns, _aseg_dtype_code, width_specifier, decimal_places = (
                decode_aseg_gdf_format(array_variable.aseg_gdf_format)
            )
...
def fix_field_precision(
    ...
    """
    logger.debug(
        "data_array: {}, current_dtype: {}, decimal_places: {}".format(
            data_array, current_dtype, decimal_places
        )
    )
...
difference_array\n{}\ndecimal_places: {}\ndifference count: {}\ndifference values: ".format(
                ...
                logger.debug(
                    "Maximum error converting from {} to {}: {}".format(
                        current_dtype,
                        smaller_dtype,
                ...
                if np.any(
                    np.logical_or(
                        (smaller_exponent != data_exponent),
                        (
                            np.abs(data_mantissa - smaller_mantissa)
                    ...
                    logger.debug(
                        "Maximum mantissa difference: {}".format(
                            np.nanmax(np.abs(data_mantissa - smaller_mantissa))
                        )
                    )
                    logger.debug(
                        "Maximum exponent difference: {}".format(
                            np.nanmax(np.abs(smaller_exponent - data_exponent))
                        )
                    )
                    (
                        aseg_gdf_format,
                        ...
                        decimal_places,
                        python_format,
                    ) = variable2aseg_gdf_format(smaller_array, decimal_places)
                    ...
                    if fill_value is not None:
                        ...
                        if np.any(no_data_mask):
                            ...
                            if np.any(
                                data_array[~no_data_mask]
                                == reduced_precision_fill_value
                            ):
                                logger.debug(
                                    "Reduced precision fill value of {} is ambiguous".format(
                                        reduced_precision_fill_value
                                    )
                                )
                                ...
                                logger.debug(
                                    "fill_value precision reduced from {} to {}".format(
...
def truncate(fill_value, data_array, no_data_mask, width_specifier, decimal_places):
    ...
    try:
        ...
        logger.debug(
            "Unable to truncate fill value from {} to {} ({}). Keeping original value.".format(
                fill_value, truncated_fill_value, e
            )
        )
        return fill_value
