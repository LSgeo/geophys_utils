...
class NetCDFVariableNational(object):
    ...
    def __init__(
        ...
        self.variable_parameters = dict(
            variable_parameters or NetCDFVariableNational.DEFAULT_VARIABLE_PARAMETERS
        )
        ...
        if self.dimensions:
            ...
            if fill_value is not None and not self.variable_parameters.get(
                "fill_value"
            ):
            ...
            if self.chunk_size and not self.variable_parameters.get("chunksizes"):
                self.variable_parameters["chunksizes"] = [
                    self.chunk_size for _dimension_name in self.dimensions
                ]
    ...
    def create_var_in_dataset(self, nc_output_dataset):
        ...
        def check_array_shape():
            ...
            logger.debug(
                "nc_output_dataset.dimensions: {}".format(nc_output_dataset.dimensions)
            )
            assert set(self.dimensions) <= set(
                list(nc_output_dataset.dimensions.keys())
            ), "Invalid dimension(s) specified: {} not in {}.".format(
                self.dimensions, nc_output_dataset.dimensions.keys()
            )

            expected_shape = tuple(
                [
                    nc_output_dataset.dimensions[dimension_name].size
                    for dimension_name in self.dimensions
                ]
            )

            assert (self.data is None) or (
                self.data.shape == expected_shape
            ), "Invalid {} array shape for specified dimension(s). Expected {}, got {}.".format(
                self.short_name, expected_shape, self.data.shape
            )
        ...
        output_variable = nc_output_dataset.createVariable(
            self.short_name, self.dtype, self.dimensions, **variable_parameters
        )
        # Set data values
...
class ToNetCDFConverterNational(object):
    ...
    def __init__(
        ...
        self.nc_output_dataset = netCDF4.Dataset(
            nc_out_path, mode=netcdf_mode, clobber=True, format=netcdf_format
        )
    ...
    def build_dim_index_variable(
        ...
        # =======================================================================
        assert (
            dimension_name in self.nc_output_dataset.dimensions.keys()
        ), "Invalid dimension name"

        dimension_variable_attributes = {"long_name": long_name, "units": units}

        if standard_name:
            dimension_variable_attributes["standard_name"] = standard_name

        increment = (max_value - min_value) / self.nc_output_dataset.dimensions[
            dimension_name
        ].size
    ...
    def build_crs_variable(self, crs, grid_dimensions=None):
        ...
        def get_geotransform(grid_dimensions):
            ...
            cell_sizes = [
                (
                    self.nc_output_dataset.variables[grid_dimensions[dim_index]][-1]
    ...
    def convert2netcdf(self):
        ...
        for dimension_name, dimension_size in iter(self.get_dimensions().items()):
            self.nc_output_dataset.createDimension(
                dimname=dimension_name, size=dimension_size
            )
        ...
        if not self.national:
            ...
            logger.debug(
                "self.get_global_attributes(): {}".format(self.get_global_attributes())
            )
