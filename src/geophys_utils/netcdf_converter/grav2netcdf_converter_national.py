...
import numpy as np
import cx_Oracle
from geophys_utils.netcdf_converter import (
...
class Grav2NetCDFConverter(ToNetCDFConverterNational):
    ...
    def get_all_ellipsoiddatum_values_table_from_point_data(self):
        """Search each survey and retrieve the dinstict results for ellidpoisd datum.
    ...
    def get_keys_and_values_table(self, table_name: str):
        ...
        if table_name == "ELLIPSOIDHGTDATUM":
            # return {"GRS80": "GRS80"}
            sql_statement = self.sql_strings_dict_from_yaml[
    ...
    def get_value_for_key(
        ...
        sql_statement = "select {0} from gravity.{1} where {2} = '{3}'".format(
            value_column, table_name, key_column, cleaned_key
        )
        query_result = self.cursor.execute(sql_statement)
    ...
    def get_survey_metadata(self):
        ...
        formatted_sql = self.sql_strings_dict_from_yaml["get_survey_metadata"].format(
            self.survey_id
        )
    ...
    def get_national_survey_metadata(self):
        ...
        field_names = [field_desc[0] for field_desc in query_result.description]
        zipped_data_dicts = [
            dict(zip(field_names, row)) for row in query_result.fetchall()
        ]
        ...
        for survey_metadata_dict in zipped_data_dicts:
            survey_metadata_dict["DURATION"] = str(
                survey_metadata_dict["ENDDATE"] - survey_metadata_dict["STARTDATE"]
            )
    ...
    def append_data2netcdf(self):
        logger.debug(
            "Running append_data2netcdf function on survey: " + str(self.survey_id)
        )
        ...
        for field_name, field_value in Grav2NetCDFConverter.settings[
            "field_names"
        ].items():
            if field_name in [
                "Obsno",
                ...
                data_list = [
                    x[0] for x in self.cursor
                ]  # get the first index. Otherwise each point is within its own tuple.

                var = self.nc_output_dataset.variables[
                    (
                        field_value.get("standard_name") or field_value["short_name"]
                    ).lower()
                ]
            ...
            if field_name in [
                ...
                key_value_dict = self.get_keys_and_values_table(
                    field_value.get("lookup_table")
                )
                ...
                lookup_dict = {
                    lookup_key: lookup_key_list.index(lookup_key)
                ...
                transformed_data_list = [
                    lookup_dict.get(lookup_key) for lookup_key in data_list
                ]
    ...
    def get_ellipsoid_height_datum_keys(self):  # only one currently, GRS80
        formatted_sql = self.sql_strings_dict_from_yaml[
            "get_ellipsoidhgt_datums_lookup"
        ]
        query_result = self.cursor.execute(formatted_sql)
        keys_and_values_dict = OrderedDict()
        ...
        self.elipsoid_height_datums = [
            datum[0] for datum in survey_row if datum[0] is not None
        ]
    ...
    def get_survey_wide_value_from_obs_table(self, field):
        ...
        for result in query_result:
            ...
            assert (
                value is None or result[0] == value or result[0] is None
            ), "Variant value found in survey-wide column {}".format(field)
        return value
    ...
    def get_global_attributes(self):
        ...
        field_names = [
            field_desc[0] for field_desc in start_date_query_result.description
        ]
        start_date_zipped_data = dict(
            zip(field_names, start_date_query_result.fetchone())
        )
        earliest_start_date = start_date_zipped_data["STARTDATE"]

        end_date_sql = self.sql_strings_dict_from_yaml[
            "get_national_survey_metadata_order_by_enddate_desc"
        ]
        end_date_query_result = self.cursor.execute(end_date_sql)
        field_names = [
            field_desc[0] for field_desc in end_date_query_result.description
        ]
        ...
        metadata_dict = {
            ...
            "elevation_accuracy_min": np.min(
                self.nc_output_dataset.variables["gndelevacc"]
            ),
            "elevation_accuracy_max": np.max(
                self.nc_output_dataset.variables["gndelevacc"]
            ),
        ...
        try:
            ...
            coordinates = np.array(
                list(
                    zip(
                        self.nc_output_dataset.variables["longitude"][:],
            ...
            if len(coordinates) >= 3:
                ...
                metadata_dict["geospatial_bounds"] = (
                    "POINT(("
                    + " ".join(["%.4f" % ordinate for ordinate in coordinates[0]])
                    + "))"
                )
        except:
    ...
    def get_dimensions(self):
        ...
        formatted_sql2 = self.sql_strings_dict_from_yaml[
            "sql_get_count_surveyids"
        ...
        for field_value in Grav2NetCDFConverter.settings["field_names"].values():
            if field_value.get("lookup_table"):
                lookup_dict = self.get_keys_and_values_table(
                    field_value["lookup_table"]
                )
    ...
    def variable_generator(self):
        ...
        def handle_key_value_cases(field_yml_settings_dict, lookup_table_dict):
            ...
            logger.debug(
                ...
                + str(field_value)
                + " and lookup_table_dict: "
                + str(lookup_table_dict)
            )
            ...
            # transform the data_list into the mapped value.
            transformed_data_list = [
                lookup_dict.get(lookup_key) for lookup_key in field_data_array
            ]
            ...
            converted_attributes_dict = {
                lookup_table_dict[key]: value
                for key, value in lookup_table_dict.items()
            }
        ...
        def build_attribute_dict_and_data_list_of_variables(field_yml_settings_dict):
            ...
            for value in list_of_possible_value:
                ...
                if field_yml_settings_dict.get(value):
                    ...
                    if value == "lookup_table":
                        logger.debug(
                            "Converting "
                            + str(value)
                            + "string keys to int8 with 0 as 1st index"
                        )
                        converted_data_list, converted_key_value_dict = (
                            handle_key_value_cases(
                                field_yml_settings_dict,
                                self.get_keys_and_values_table(
                                    field_yml_settings_dict.get("lookup_table")
                                ),
                            )
                        )

                        logger.debug(
                            "Adding converted lookup table as variable attribute..."
                        )
                        ...
                        # print(converted_data_list)
                        print(
                            "before converted data list: {}".format(converted_data_list)
                        )
                        ...
                        converted_data_array = np.array(
                            converted_data_list, field_yml_settings_dict["dtype"]
                        )
                    ...
                    if value == "dem" or value == "datum":
                        ...
                        if field_yml_settings_dict.get("short_name") == "Grav":
                            ...
                            attributes_dict[value] = self.get_value_for_key(
                                "DESCRIPTION", "GRAVDATUMS", "GRAVDATUM", gravdatum_key
                            )
                        # while TCDEM and ELLIPSOIDHGTDATUM do not
                        else:
                            attributes_dict[value] = (
                                self.get_survey_wide_value_from_obs_table(
                                    field_yml_settings_dict.get(value)
                                )
                            )
                        ...
                        try:
                            ...
                            logger.debug(
                                'attributes_dict["{}"] = {}'.format(
                                    value, field_yml_settings_dict[value]
                                )
                            )
                        except:
                            print("no go")
                # if the value isn't in the list of accepted attributes
                    ...
                    logger.debug(
                        str(value)
                        + " is not found in yaml config or is not set as an accepted attribute."
                    )
            ...
            # else get the non converted data and return it in an numpy array and the and the attribute dict too
                ...
                data_array = np.array(
                    get_data(field_yml_settings_dict),
                    dtype=field_yml_settings_dict["dtype"],
                )
        ...
        yield NetCDFVariableNational(
            short_name="survey_metadata",
        ...
        yield NetCDFVariableNational(
            short_name="survey_metadata_index",
            data=np.array([self.survey_index] * self.point_count, dtype="int32"),
            dimensions=["point"],
            fill_value=int("-9999"),
            attributes={
                "long_name": "zero-based index of value in survey_metadata",
        ...
        for field_name, field_value in Grav2NetCDFConverter.settings[
            ...
            data, attributes = build_attribute_dict_and_data_list_of_variables(
                field_value
            )
            ...
            if field_value.get("lookup_table"):
                ...
                lookup_table_dict = self.get_keys_and_values_table(
                    field_value["lookup_table"]
                )
                ...
                index_attributes["long_name"] = (
                    "zero-based index of value in " + dim_name
                )
...
def main():
    ...
    yaml_sql_settings = yaml.safe_load(
        open(os.path.splitext(__file__)[0] + "_sql_strings.yml")
    )
    ...
    survey_id_list = [
        re.search("\d+", survey_row[0]).group() for survey_row in survey_cursor
    ]
    ...
    for survey in survey_id_list:
        ...
        if survey_index == 0:
            ...
            g2n = Grav2NetCDFConverter(
                "{}/NATIONAL_GNDGRAV.nc".format(nc_out_path),
                survey,
                survey_index,
                con,
                sql_strings_dict,
