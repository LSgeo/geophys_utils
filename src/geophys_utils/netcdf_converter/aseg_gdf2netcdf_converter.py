...
from geophys_utils.netcdf_converter.aseg_gdf_utils import (
    aseg_gdf_format2dtype,
    fix_field_precision,
    truncate,
)
...
class ASEGGDF2NetCDFConverter(ToNetCDFConverter):
    ...
    def __init__(
        ...
        def get_field_definitions():
            ...
            def parse_dfn_file(dfn_path):
                ...
                for line in dfn_file:
                    ...
                    for semicolon_split_string in [
                        semicolon_split_string.strip()
                        for semicolon_split_string in line.split(";")
                    ]:
                        for colon_split_string in [
                            colon_split_string.strip()
                            for colon_split_string in semicolon_split_string.split(":")
                        ]:
                            for comma_split_string in [
                                comma_split_string.strip()
                                for comma_split_string in colon_split_string.split(",")
                            ]:
                                definition = [
                                    equals_split_string.strip()
                                    for equals_split_string in comma_split_string.split(
                                        "="
                                    )
                                ]
                                if len(definition) == 2:
                    ...
                    logger.debug(
                        "line: {}\nkey_value_pairs: {}\npositional_value_list: {}".format(
                            line.strip(),
                            pformat(key_value_pairs),
                            pformat(positional_value_list),
                        )
                    )

                    # Column definition
                    if key_value_pairs.get("RT") in ["", "DATA"] and (
                        positional_value_list and positional_value_list[0] != "END DEFN"
                    ):
                        short_name = positional_value_list[0].lower()
                        fmt = (
                            positional_value_list[1]
                            if len(positional_value_list) >= 2
                            else None
                        )
                        units = key_value_pairs.get("UNITS") or key_value_pairs.get(
                            "UNIT"
                        )
                        long_name = key_value_pairs.get("NAME") or (
                            positional_value_list[2]
                            if len(positional_value_list) >= 3
                            else None
                        )
                        fill_value = (
                            float(key_value_pairs.get("NULL"))
                            if key_value_pairs.get("NULL") is not None
                            else None
                        )

                        # Parse format to determine columns, data type and numeric format
                        dtype, columns, width_specifier, decimal_places = (
                            aseg_gdf_format2dtype(fmt)
                        )
                        ...
                        variable_attribute_dict = {
                            attribute_name: key_value_pairs.get(key.upper())
                            for key, attribute_name in self.settings[
                                "variable_attributes"
                            ].items()
                        ...
                        if variable_attribute_dict:
                            field_definition["variable_attributes"] = (
                                variable_attribute_dict
                            )
                    ...
                    # Set CRS from projection name
                        ...
                        if (key_value_pairs.get("RT") == "PROJ") and (
                            positional_value_list[0] == "COORDSYS"
                        ):  # As per ASEG standard
                            projection_name = key_value_pairs.get("NAME")
                            if projection_name:
                                self.spatial_ref = get_spatial_ref_from_wkt(
                                    projection_name
                                )
                                logger.debug(
                                    "CRS set from .dfn file COORDSYS COMMENT attribute {}".format(
                                        projection_name
                                    )
                                )
                                break  # Nothing more to do
                        elif (key_value_pairs.get("RT") == "PROJ") and (
                            positional_value_list[0] == "PROJNAME"
                        ):  # Non-standard
                            ...
                            if projection_name:
                                ...
                                logger.debug(
                                    "CRS set from .dfn file PROJNAME NAME attribute {}".format(
                                        projection_name
                                    )
                                )
                                break  # Nothing more to do
                        elif (key_value_pairs.get("RT") == "PROJ") and (
                            positional_value_list[0] == "DATUM"
                        ):  # Unprojected
                            projection_name = key_value_pairs.get("NAME")
                            if projection_name:
                                self.spatial_ref = get_spatial_ref_from_wkt(
                                ...
                                logger.debug(
                                    "CRS set from .dfn file DATUM NAME attribute {}".format(
                                        projection_name
                                    )
                                )
                                break  # Nothing more to do
            ...
            if self.settings.get("field_definitions"):
                for field_definition in self.field_definitions:
                    overriding_field_definition = self.settings[
                        "field_definitions"
                    ].get(field_definition["short_name"])
            ...
            logger.debug(
                "self.field_definitions: {}".format(pformat(self.field_definitions))
            )
            ...
            logger.debug(
                "self.spatial_ref: {}".format(self.spatial_ref.ExportToPrettyWkt())
            )
        ...
        def read_data_file():
            ...
            def create_nc_cache():
                ...
                self._nc_cache_dataset.createDimension(
                    dimname="rows", size=None
                )  # Unlimited size
                ...
                for field_definition in self.field_definitions:
                    ...
                    if columns == 1:  # 1D variable
                        ...
                        field_dimension_name = (
                            short_name + "_dim"
                        )  # Default 2nd dimension name

                        # Look up any dimension name(s) for this variable from settings
                        override_definition = self.settings["field_definitions"].get(
                            short_name
                        )
                        ...
                        if override_definition:
                            ...
                            if override_dimensions:
                                if type(override_dimensions) == list:
                                    ...
                                    field_dimension_name = (
                                        override_dimensions + "_dim"
                                    )  # Single dimension

                        # Create dimension if it doesn't already exist
                        if (
                            field_dimension_name
                            not in self._nc_cache_dataset.dimensions.keys()
                        ):
                            self._nc_cache_dataset.createDimension(
                                dimname=field_dimension_name, size=columns
                            )
                    ...
                    self._nc_cache_dataset.createVariable(
                        varname=short_name,
                ...
                try:  # Do a sync now to force an error if there are dimension/variable naming conflicts
                    ...
                    logger.debug(
                        self._nc_cache_dataset.dimensions,
                        self._nc_cache_dataset.variables,
                    )
                    raise

                logger.debug(
                    "Created temporary cache file {}".format(self.nc_cache_path)
                )
            ...
            def cache_chunk_list(chunk_list, start_row):
                ...
                for field_index in range(len(self.field_definitions)):
                    ...
                    chunk_array = np.array(
                        [row_list[field_index] for row_list in chunk_list]
                    )
            ...
            def read_fixed_length_fields(line):
                ...
                for field_definition in self.field_definitions:
                    ...
                    for _column_offset in range(columns):
                        end_char_index = (
                            start_char_index + field_definition["width_specifier"]
                        )
                        ...
                        if (
                            ...
                            end_char_index = start_char_index + len(
                                value_string
                            )  # Adjust character offset for next column
                        ...
                        try:
                            ...
                            logger.warning(
                                'Unable to convert "{}" field value "{}" to type {}'.format(
                                    short_name, value_string, dtype
                                )
                            )
                ...
                if line_column_count != self.column_count:
                    logger.warning(
                        "Invalid number of columns found: Expected {}, found {}. Skipping line".format(
            ...
            for line in dat_in_file:
                ...
                if POINT_LIMIT and self.total_points >= POINT_LIMIT:
                    logger.debug(
                        "Truncating input for testing after {} points".format(
                            POINT_LIMIT
                        )
                    )
                    break
        ...
        def modify_field_definitions():
            ...
            while field_definition_index < len(self.field_definitions) - 1:
                ...
                logger.debug(
                    "field_definition_index: {}, field_name: {}".format(
                        field_definition_index, field_name
                    )
                )

                if field_name in self.settings["dimension_fields"]:
                    self.dimensions[field_name] = int(
                        cache_variable[0]
                    )  # Read value from first line
                    # logger.debug('self.dimensions["{}"] set to {}'.format(field_name, cache_variable[0]))
                    field_definition["column_start_index"] = (
                        None  # Do not output this column
                    )
                ...
                if match:  # format string starts with integers - 2D variable
                    ...
                    default_dimension_names = field_definition.get(
                        "dimensions"
                    )  # Look for default dimension name
                    ...
                    if (
                    ...
                    ):  # Default dimension name(s) found from settings
                        if (
                            type(default_dimension_names) == str
                        ):  # Only one dimension specified as a string
                            actual_dimension_size = self.dimensions.get(
                                default_dimension_names
                            )
                            logger.debug(
                                "actual_dimension_size: {}".format(
                                    actual_dimension_size
                                )
                            )
                            if (
                                actual_dimension_size is None
                            ):  # Default dimension not already defined - create it
                                self.dimensions[default_dimension_names] = (
                        ...
                        # ideally be re-written
                            ...
                            type(default_dimension_names) == list
                        ):  # Multiple part-dimensions specified
                            assert set(default_dimension_names) <= set(
                                self.dimensions.keys()
                            ), "Dimensions {} not all defined in {}".format(
                                default_dimension_names, self.dimensions.keys()
                            )
                            combined_dimension_size = sum(
                                [
                                    self.dimensions[key]
                            ...
                            for part_dimension_name in default_dimension_names:
                                ...
                                new_field_definition = dict(
                                    field_definition
                                )  # Copy original field definition
                                new_field_definition["cache_variable_name"] = (
                                    field_definition["short_name"]
                                )
                                new_field_definition["short_name"] += (
                                    "_by_" + part_dimension_name
                                )
                                ...
                                field_definition_index += 1
                                logger.debug(
                                    "Inserting new part-dimension field definition for {} at index {}".format(
                                        new_field_definition["short_name"],
                                        field_definition_index,
                                    )
                                )
                                self.field_definitions.insert(
                                    field_definition_index, new_field_definition
                                )
                        ...
                        if match:
                            dimension_name = match.group(
                                2
                            )  # This probably won't happen
                        else:
                            dimension_name = (
                                field_name + "_dim"
                            )  # Nasty, but it will get the job done.
                        ...
                        logger.debug(
                            "Variable {} has dimension {} of size {}".format(
                                field_name, dimension_name, field_dimension_size
                            )
                        )
        ...
        def fix_all_field_precisions():
            ...
            for field_definition in self.field_definitions:
                ...
                if cache_variable_name:  # Part dimension  - shared cache variable
                    cache_variable = self._nc_cache_dataset.variables[
                        cache_variable_name
                    ]
                ...
                if type(data_array) == np.ma.core.MaskedArray:
                    ...
                    )  # Use masked array fill value instead of any provided value
                ...
                precision_change_result = fix_field_precision(
                    data_array,
                    dtype,
                ...
                if precision_change_result:
                    self.info_output(
                        "Datatype for variable {} changed from {} to {}".format(
                            short_name, dtype, precision_change_result[1]
                        )
                    )
                    ...
                    variable_attributes = (
                        field_definition.get("variable_attributes") or {}
                    )
                    ...
                    logger.debug(
                        "Datatype for variable {} not changed".format(short_name)
                    )
                    # Try truncating original fill value
                    modified_fill_value = truncate(
                        fill_value,
                        data_array,
                        no_data_mask,
                ...
                if (
                    ...
                    data_array[
                        np.logical_or((data_array == fill_value), np.isnan(data_array))
                    ] = modified_fill_value
                    cache_variable[:] = data_array

                    self.info_output(
                        "{} fill_value changed from {} to {} for format {}".format(
                            short_name,
                            fill_value,
                            modified_fill_value,
        ...
        self.settings_path = settings_path or os.path.join(
            os.path.dirname(__file__), "aseg_gdf_settings.yml"
        )
        try:
        ...
        if True:  # try:
            ...
            logger.error(
                "Unable to create ASEGGDF2NetCDFConverter object: {}".format(e)
            )
            self.__del__()
    ...
    def get_raw_data(self, variable_name):
        ...
        try:
            ...
            try:  # Try part dimension with shared cache variable
                ...
                return self._nc_cache_dataset.variables[
                    field_definition["cache_variable_name"]
                ][
                    :,
                    field_definition["column_offset"] : field_definition[
                        "column_offset"
                    ]
                    + field_definition["columns"],
                ]
            except:
                return None
    ...
    def get_global_attributes(self):
        ...
        metadata_dict = {
            "title": "Dataset read from ASEG-GDF file {}".format(
                os.path.basename(self.dat_path)
            ),
            ...
            or np.min(self.get_raw_data("dtm")),
            "geospatial_vertical_max": np.max(self.get_raw_data("elevation"))
            or np.min(self.get_raw_data("dtm")),
            "geospatial_vertical_units": "m",
            "geospatial_vertical_resolution": "point",
            "geospatial_vertical_positive": "up",
            "history": "Converted from ASEG-GDF file {} using definitions file {}".format(
                self.dat_path, self.dfn_path
            ),
        ...
        try:
            ...
            metadata_dict["geospatial_bounds"] = (
                "POLYGON(("
                + ", ".join(
                    [
    ...
    def variable_generator(self):
        ...
        def index_variable_generator():
            ...
            yield NetCDFVariable(
                short_name="{}_start_index".format(field_definition["short_name"]),
                data=index_start_indices,
                dimensions=[field_definition["short_name"]],
                fill_value=-1,
                attributes={
                    "long_name": "zero-based index of the first point in each {}".format(
                        field_definition["short_name"]
                    )
                },
                dtype=np.int32,
            ...
            yield NetCDFVariable(
                short_name="{}_point_count".format(field_definition["short_name"]),
                data=index_point_counts,
                dimensions=[field_definition["short_name"]],
                fill_value=-1,
                attributes={
                    "long_name": "number of points in each {}".format(
                        field_definition["short_name"]
                    )
                },
                dtype=np.int32,
        ...
        def lookup_variable_generator():
            ...
            yield NetCDFVariable(
                ...
                fill_value=-1,
                attributes={
                    "long_name": "zero-based index of value in {}".format(short_name),
                    "lookup": short_name,
                },
                dtype=(
                    np.int8
        ...
        for field_definition in self.field_definitions:
            ...
            lookup_array, index_start_indices, index_array, index_point_counts = (
                np.unique(
                    raw_data, return_index=True, return_inverse=True, return_counts=True
                )
            )

            unique_value_count = lookup_array.shape[0]
            logger.debug(
                "{} unique values found for {}".format(unique_value_count, short_name)
            )
            ...
            if (
                (field_definition["columns"] == 1)
                and (unique_value_count == 1)
                and (index_point_counts[0] == raw_data.shape[0])
            ):
            ...
            # Process string fields or designated lookup fields as lookups
                ...
                field_definition["format"].startswith("A")  # String field
                or short_name
                in self.settings["lookup_fields"]  # Designated lookup field
            ):
            ...
            # =======================================================================
                ...
                if short_name == "resistivity":
                    ...
                    field_attributes = {
                        "long_name": "Absolute uncertainty of layer conductivity",
                        "units": "S/m",
                    }
                    data_array = self.get_raw_data(
                        "resistivity_uncertainty"
                    ) / self.get_raw_data("resistivity")
                    fill_value = 0
                    # ===========================================================
                    # data_array[bad_data_mask] = fill_value
                    # ===========================================================
    ...
    def postprocess_netcdf(self):
        ...
        transverse_mercator_var = self.nc_output_dataset.variables.get(
            "transverse_mercator"
        )
        ...
        if (
            transverse_mercator_var is not None
            and crs_var is None
            and set(["easting", "northing"])
            <= set(self.nc_output_dataset.variables.keys())
        ):
            # Build GDA94 crs variable and write it to self.nc_output_dataset
            self.info_output(
                "Creating crs, longitude and latitude variables for unprojected CRS"
            )
            point_count = self.nc_output_dataset.dimensions["point"].size
            ...
            gda94_coords = transform_coords(
                utm_coords, transverse_mercator_var.spatial_ref, default_crs_wkt
            )

            # Create and write crs variable
            self.info_output("Creating new crs variable for unprojected CRS")
            self.build_crs_variable(
                get_spatial_ref_from_wkt(default_crs_wkt)
...
def main():
    ...
    def get_args():
        ...
        parser.add_argument(
            "-f", "--dfn", help="Path to .dfn file", type=str, dest="dfn_in_path"
        )
        parser.add_argument(
            "-s",
            "--settings",
    ...
    )  # Logger for base class ToNetCDFConverter
...
Usage: python {} <options> <dat_in_path> [<nc_out_path>]".format(
    ...
    dat_in_path = args.positional_args[
        0
    ]  # 'C:\\Temp\\Groundwater Data\\ord_bonaparte_nbc_main_aquifer_clipped.dat'
    ...
    if True:  # try:
        ...
        try:
            ...
            logger.debug(
                "Unable to delete ASEGGDF2NetCDFConverter object: {}".format(e)
            )
        ...
        try:
            ...
            logger.debug(
                "Unable to delete incomplete netCDF file {}: {}".format(nc_out_path, e)
            )
...
if __name__ == "__main__":
    ...
    if not logger.handlers:
        ...
        aseg_gdf_utils_logger = logging.getLogger(
            "geophys_utils.netcdf_converter.aseg_gdf_utils"
        )
