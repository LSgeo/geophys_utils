...
class RowCache(object):
    ...
    def read_points(self, start_index, end_index, point_mask=None):
        ...
        def expand_point_lookup_variable(
            ...
            logger.debug(
                "indexing_variable = {}, lookup_variable = {}".format(
                    indexing_variable, lookup_variable
                )
            )

            return lookup_variable[
                indexing_variable[start_index:end_index].astype(np.int)
            ]
        ...
        def expand_line_lookup_variable(lookup_variable_name):
            ...
            assert lookup_variable.dimensions == (
                "line",
            ), 'Variable "{}" has dimensions {}'.format(
        ...
        for field_definition in self.field_definitions:
            ...
            if (
                ...
                logger.debug(
                    "variable_name = {}, variable.dimensions = {}".format(
                        variable_name, variable.dimensions
                    )
                )
                # Point-wise lookup array variable
                if (variable.dimensions[0] == "point") and variable_name.endswith(
                    "_index"
                ):
                    self.cache[short_name] = expand_point_lookup_variable(
                        variable_name
                    )[subset_mask]

                # Deal with line-indexed variables (like maybe "flight")
                if (
                    variable.dimensions == ("line",)
                ) and variable_name != "line":  # "line" variable will be dealt with through "line_index" in above case
                    self.cache[short_name] = expand_line_lookup_variable(variable_name)[
                        subset_mask
                    ]
                ...
                raise BaseException(
                    "Invalid dimensionality for variable {}".format(variable_name)
                )
        ...
        for index in range(self.index_range):
            ...
            for field_definition in self.field_definitions:
                ...
                if field_definition["columns"] == 1:  # Element from 1D variable
                    ...
                    raise BaseException(
                        "Invalid dimensionality for variable {}".format(
...
class NetCDF2ASEGGDFConverter(object):
    ...
    def __init__(
        ...
        assert (
            "point" in self.nc_dataset.dimensions.keys()
        ), "No point dimension defined in netCDF dataset"
    ...
    def convert2aseg_gdf(self, dat_out_path=None, dfn_out_path=None, point_mask=None):
        ...
        def get_field_definitions():
            ...
            for variable_name, variable in self.nc_dataset.variables.items():
                ...
                if not variable.shape and (
                    variable_name in ["crs", "transverse_mercator"]
                    or re.match("ga_.+_metadata", variable_name)
                ):
                    continue

                # Resolve lookups - use lookup variable instead of index variable
                if variable_name.endswith("_index") | hasattr(
                    variable, "lookup"
                    ...
                    try:
                        ...
                        short_name = re.sub(
                            "_index$", "", variable_name, re.IGNORECASE
                        )  # Use associated variable name

                    try:
                        lookup_variable = self.nc_dataset.variables[short_name]
                    except:
                        logger.warning(
                            'Unable to access lookup variable "{}" for index variable "{}"'.format(
                    ...
                    (
                        ...
                        decimal_places,
                        python_format,
                    ) = variable2aseg_gdf_format(lookup_variable)
                    ...
                    (
                        ...
                        decimal_places,
                        python_format,
                    ) = variable2aseg_gdf_format(variable)

                # Map variable name to standard ASEG-GDF field name if required
                short_name = (
                    self.settings["aseg_field_mapping"].get(short_name) or short_name
                )
                ...
                variable_attribute_dict = {
                    attribute_name: variable_attributes.get(key.upper())
        ...
        def write_dfn_file():
            ...
            def write_defns(dfn_file):
                ...
                for field_definition in self.field_definitions:
                    ...
                    if units:
                        optional_attribute_list.append(
                            "UNITS={units}".format(units=units)
                        )
                    ...
                    if fill_value is not None:
                        optional_attribute_list.append(
                            "NULL="
                            + field_definition["python_format"]
                            .format(fill_value)
                            .strip()
                        )
                    ...
                    if variable_attributes:
                        for aseg_gdf_attribute, netcdf_attribute in self.settings[
                            "variable_attributes"
                        ].items():
                            attribute_value = variable_attributes.get(netcdf_attribute)
                            if attribute_value is not None:
                                optional_attribute_list.append(
                                    "{aseg_gdf_attribute}={attribute_value}".format(
                ...
                # Write 'END DEFN'
                self.write_record2dfn_file(
                    dfn_file, rt="", name="END DEFN", aseg_gdf_format=""
                )
            ...
            def write_proj(dfn_file):
                ...
                if self.spatial_ref.IsProjected():
                    ...
                    projection_method = self.spatial_ref.GetAttrValue(
                        "projection"
                    ).replace("_", " ")
                ...
                self.write_record2dfn_file(
                    dfn_file, rt="PROJ", name="RT", aseg_gdf_format="A4"
                )
                ...
                # Write 'END DEFN'
                self.write_record2dfn_file(
                    dfn_file, rt="PROJ", name="END DEFN", aseg_gdf_format=""
                )
        ...
        self.dat_out_path = (
            dat_out_path or os.path.splitext(self.netcdf_in_path)[0] + ".dat"
        )
...
Usage: python {} <options> <nc_in_path> [<dat_out_path>]".format(
    ...
    netcdf2aseg_gdf_converter = NetCDF2ASEGGDFConverter(
        nc_in_path, crs_string=args.crs, verbose=args.verbose
    )
    ...
    netcdf2aseg_gdf_converter.convert2aseg_gdf(dat_out_path, dfn_out_path, point_mask)
