...
class NetCDFUtils(object):
    ...
    ]  # TODO: See if we can make this exhaustive
    ...
    def copy(
        ...
        nc_output_dataset = netCDF4.Dataset(
            nc_out_path, mode="w", clobber=True, format=nc_format
        )
        ...
        try:
            ...
            for variable_name, variable in filtered_variables.items():
                ...
                for dimension_index in range(len(variable.dimensions)):
                    ...
                    if dest_dim_size.get(
                        dimension_name
                    ):  # We have already configured this dimension
                        continue

                    dim_mask = dim_mask_dict.get(dimension_name)
                    if dim_mask is None:
                        dim_mask = np.ones(
                            shape=(variable.shape[dimension_index],), dtype=bool
                        )
                    else:
                        assert dim_mask.shape == (
                            source_dimension.size,
                        ), "Dimension mask must be a 1D boolean mask of size {}".format(
                            source_dimension.size
                        )
                    ...
                    dest_dim_size[dimension_name] = np.count_nonzero(
                        dim_mask
                    )  # Update sizes to take masks into account
            ...
            for dimension_name, dimension in self.netcdf_dataset.dimensions.items():
                if dimension_name in dims_used:  # Discard unused dimensions
                    logger.debug(
                        "Copying dimension %s of length %d"
                        % (dimension_name, dest_dim_size[dimension_name])
                    )
                    nc_output_dataset.createDimension(
                        dimension_name,
                        (
            ...
            for variable_name, input_variable in filtered_variables.items():
                dtype = (
                    datatype_map_dict.get(str(input_variable.datatype))
                    or input_variable.datatype
                )
                ...
                [
                    var_options.pop(key, None)
                    for key in ("zlib", "szip", "zstd", "bzip2", "blosc")
                ]
                ...
                if chunking and chunking != "contiguous":
                    ...
                    and variable_options_dict.get(variable_name)
                    and variable_options_dict.get(variable_name).get("chunksizes")
                ):  # TODO: Improve this
                    # If input variable is unchunked 2D and output chunking is specified - assume row chunking for input
                    input_variable_chunking = [
                        1,
                        dest_dim_size[input_variable.dimensions[1]],
                    ]
                else:
                ...
                if var_options.get("chunksizes"):
                    for dimension_index in range(len(input_variable.dimensions)):
                        var_options["chunksizes"][dimension_index] = min(
                            var_options["chunksizes"][dimension_index]
                            or dest_dim_size[
                ...
                options_string = (
                    " with options: %s"
                    % ", ".join(["%s=%s" % item for item in var_options.items()])
                    if var_options
                    else ""
                )
                logger.debug(
                    "Copying variable %s from datatype %s to datatype %s%s"
                    % (variable_name, input_variable.datatype, dtype, options_string)
                )
                # Create output variable using var_options to specify output options
                output_variable = nc_output_dataset.createVariable(
                    variable_name, dtype, input_variable.dimensions, **var_options
                )

                # Copy variable attributes
                logger.debug(
                    "\tCopying %s attributes: %s"
                    % (variable_name, ", ".join(input_variable.ncattrs()))
                )
                output_variable.setncatts(
                    {
                        k: input_variable.getncattr(k)
                        for k in input_variable.ncattrs()
                ...
                if variable_name not in empty_var_list:
                    ...
                    if input_variable.shape:  # array
                        ...
                        logger.debug(
                            "\tCopying {} array data of shape {}".format(
                                variable_name,
                                tuple(
                                    [
                                        input_variable_slices[dimension_index].stop
                                        - input_variable_slices[dimension_index].start
                                        for dimension_index in range(
                                            len(input_variable.dimensions)
                                        )
                                    ]
                        ...
                        variable_masks = tuple(
                            [
                                dim_mask_dict.get(
                                    dimension_name
                                )  # Use specified mask if it exists, None otherwise
                        ...
                        if not input_variable_chunking:
                            ...
                            logger.debug(
                                "input_variable.shape = {}".format(input_variable.shape)
                            )
                            ...
                            if (
                                not array_item_size and input_variable.dtype == str
                            ):  # and len(input_variable.dimensions) == 1
                                array_item_size = max(
                                    [
                                        sys.getsizeof(element)
                                        for element in input_variable[:]
                                    ]
                                )
                            logger.debug(
                                "array_item_size (in bytes) = {}".format(
                                    array_item_size
                                )
                            )
                            ...
                            logger.debug(
                                "smallest_piece_sizes = {}".format(smallest_piece_sizes)
                            )
                            ...
                            piece_counts = tuple(
                                [
                                    int(
                                        math.ceil(
                                            float(
                                                self.netcdf_dataset.dimensions[
                            ...
                            largest_piece_count_index = piece_counts.index(
                                max(piece_counts)
                            )
                            smallest_piece_bytes = array_item_size * reduce(
                                lambda x, y: x * y, smallest_piece_sizes
                            )
                            piece_sizes[largest_piece_count_index] = min(
                                input_variable.shape[largest_piece_count_index],
                                int(
                                    smallest_piece_sizes[largest_piece_count_index]
                                    * (self.max_bytes // smallest_piece_bytes)
                                ),
                            )

                            piece_bytes = np.dtype(
                                input_variable.dtype
                            ).itemsize * reduce(lambda x, y: x * y, piece_sizes)
                            logger.debug(
                                "piece_sizes = {}, bytes = {}".format(
                                    piece_sizes, piece_bytes
                                )
                            )
                            ...
                            logger.debug(
                                "\tCopying {} pieces of dimensions {}".format(
                                    " x ".join(
                                        [
                                            str(piece_count)
                                    ...
                                    " x ".join(
                                        [str(piece_size) for piece_size in piece_sizes]
                                    ),
                                )
                            )
                            ...
                            for piece_indices in itertools.product(
                                *[
                                    range(
                                        piece_index_ranges[dimension_index][0],
                                ...
                                offset_piece_indices = tuple(
                                    [
                                        ...
                                        for indices in zip(
                                            ...
                                            [
                                                piece_index_range[0]
                                                for piece_index_range in piece_index_ranges
                                            ],
                                        )
                                    ]
                                )
                                logger.debug(
                                    "\t\tCopying piece {}/{}".format(
                                        offset_piece_indices, piece_counts
                                    )
                                )
                                    ...
                                    [
                                        slice(
                                            ...
                                            min(
                                                ...
                                                (piece_indices[dimension_index] + 1)
                                                * piece_sizes[dimension_index],
                                            ),
                                        )
                                        for dimension_index in range(
                                ...
                                piece_write_slices = tuple(
                                    [
                                        (
                                            slice(
                                                np.count_nonzero(
                                                    variable_masks[dimension_index][
                                                        input_variable_slices[
                                                            dimension_index
                                                        ]
                                                    ][
                                            ...
                                            if variable_masks[dimension_index]
                                            is not None
                                            else slice(
                                                ...
                                                piece_read_slices[dimension_index].stop
                                                - input_variable_slices[
                                                    dimension_index
                                                ].start,
                                ...
                                output_variable[piece_write_slices] = input_variable[
                                    piece_read_slices
                                ][piece_dim_masks]
            ...
            logger.debug(
                "Copying global attributes: %s"
                % ", ".join(self.netcdf_dataset.__dict__.keys())
            )
            ...
            logger.debug(
                "Finished copying netCDF dataset %s to %s."
                % (self.nc_path, nc_out_path)
            )
    ...
    def get_crs_attributes(self, crs):
        ...
        crs_attributes["longitude_of_prime_meridian"] = spatial_ref.GetAttrValue(
            "PRIMEM", 1
        )

        # TODO: Make this more general to follow GDAL grid dataset conventions
        if spatial_ref.IsSame(
            get_spatial_ref_from_wkt("EPSG:3577")
            ...
            crs_attributes["latitude_of_projection_origin"] = spatial_ref.GetProjParm(
                "latitude_of_origin"
            )
            crs_attributes["scale_factor_at_central_meridian"] = (
                spatial_ref.GetProjParm("scale_factor")
            )
            crs_attributes["longitude_of_central_meridian"] = spatial_ref.GetProjParm(
                "central_meridian"
            )
        ...
        logger.debug(
            "{} attributes: {}".format(crs_variable_name, pformat(crs_attributes))
        )
            ...
            if self.opendap:
                try:
                    ...
                    self._netcdf_dataset = netCDF4.Dataset(
                        self.nc_path + "#fillmismatch", mode="r"
                    )  # Work-around for _FillValue mismatch: https://github.com/Unidata/netcdf-c/issues/1299
            else:
                self._netcdf_dataset = netCDF4.Dataset(self.nc_path, mode="r")
    ...
    def crs_variable(self):
        ...
        if self._crs_variable is None:
            ...
            for crs_variable_name in NetCDFUtils.CRS_VARIABLE_NAMES:
                self._crs_variable = self.netcdf_dataset.variables.get(
                    crs_variable_name
                )
    ...
    def debug(self, debug_value):
        if self._debug != debug_value or self._debug is None:
            ...
            for base_class in self.__class__.__bases__:
                ...
                logger.debug(
                    "Logger {} set to level {}".format(base_class.__module__, log_level)
                )
...
def main():
    ...
    ncu = NetCDFUtils(args.input_path, debug=args.debug)
    ncu.max_bytes = 50000
